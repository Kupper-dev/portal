...
var require_middleware = __commonJS({
  ".next/server/src/middleware.js"() {
    "use strict";
    (self.webpackChunk_N_E = self.webpackChunk_N_E || []).push([[550], { 194: (a, b, c) => {
      "use strict";
      c.d(b, { Lf: () => k, Mh: () => j, Og: () => l, jU: () => i });
      var d = c(9148), e = c(8774), f = c(2877), g = c(9663), h = c(1967);
      async function i(a2, b2, c2) {
        if ("string" != typeof a2 || 0 !== a2.indexOf("-----BEGIN PUBLIC KEY-----")) throw TypeError('"spki" must be SPKI formatted string');
        return (0, e.sc)(a2, b2, c2);
      }
      async function j(a2, b2, c2) {
        if ("string" != typeof a2 || 0 !== a2.indexOf("-----BEGIN CERTIFICATE-----")) throw TypeError('"x509" must be X.509 formatted string');
        return (0, e.zp)(a2, b2, c2);
      }
      async function k(a2, b2, c2) {
        if ("string" != typeof a2 || 0 !== a2.indexOf("-----BEGIN PRIVATE KEY-----")) throw TypeError('"pkcs8" must be PKCS#8 formatted string');
        return (0, e.IG)(a2, b2, c2);
      }
      async function l(a2, b2, c2) {
        let e2;
        if (!(0, h.G)(a2)) throw TypeError("JWK must be an object");
        switch (b2 ??= a2.alg, e2 ??= c2?.extractable ?? a2.ext, a2.kty) {
          case "oct":
            if ("string" != typeof a2.k || !a2.k) throw TypeError('missing "k" (Key Value) Parameter value');
            return (0, d.decode)(a2.k);
          case "RSA":
            if ("oth" in a2 && void 0 !== a2.oth) throw new g.JOSENotSupported('RSA JWK "oth" (Other Primes Info) Parameter value is not supported');
            return (0, f.x)({ ...a2, alg: b2, ext: e2 });
          case "AKP":
            if ("string" != typeof a2.alg || !a2.alg) throw TypeError('missing "alg" (Algorithm) Parameter value');
            if (void 0 !== b2 && b2 !== a2.alg) throw TypeError("JWK alg and alg option value mismatch");
            return (0, f.x)({ ...a2, ext: e2 });
          case "EC":
          case "OKP":
            return (0, f.x)({ ...a2, alg: b2, ext: e2 });
          default:
            throw new g.JOSENotSupported('Unsupported "kty" (Key Type) Parameter value');
        }
      }
    }, 259: (a, b, c) => {
      "use strict";
      let d;
      c.r(b), c.d(b, { default: () => bO });
      var e, f = {};
      async function g() {
        return "_ENTRIES" in globalThis && _ENTRIES.middleware_instrumentation && await _ENTRIES.middleware_instrumentation;
      }
      c.r(f), c.d(f, { config: () => bK, middleware: () => bJ });
      let h = null;
      async function i() {
...
Learn More: https://nextjs.org/docs/messages/node-module-in-edge-runtime`;
  ...
  `);
      ...
      let { context: aw, propagation: ax, trace: ay, SpanStatusCode: az, SpanKind: aA, ROOT_CONTEXT: aB } = d = c(3817);
      class aC extends Error {
        constructor(a10, b2) {
      ...
      c(5356).Buffer, new aT(52428800, (a10) => a10.size), process.env.NEXT_PRIVATE_DEBUG_CACHE && console.debug.bind(console, "DefaultCacheHandler:"), process.env.NEXT_PRIVATE_DEBUG_CACHE && ((a10, ...b2) => {
      ...
      var bs = c(5124), bt = c(4968), bu = c(7276);
      let bv = process.env.AUTH0_ISSUER_BASE_URL?.replace("https://", "").replace("/", "") || "", bw = process.env.AUTH0_CLIENT_ID || "", bx = process.env.AUTH0_CLIENT_SECRET || "", by = process.env.AUTH0_SECRET || bx, bz = new TextEncoder().encode(by.padEnd(32, "0").substring(0, 32)), bA = process.env.NEXT_PUBLIC_BASE_PATH || "/app";
      function bB(a10) {
        if (process.env.AUTH0_BASE_URL) try {
          let a11 = new URL(process.env.AUTH0_BASE_URL);
          return `${a11.origin}${bA}`;
        } catch (a11) {
          console.error("[AuthEdge] Invalid AUTH0_BASE_URL, falling back to headers", a11);
        }
        let b2 = a10.headers, c2 = b2.get("x-forwarded-host") || b2.get("host"), d2 = b2.get("x-forwarded-proto") || "https";
        return c2 ? `${d2}://${c2}${bA}` : new URL(a10.url).origin;
      }
      async function bC(a10) {
        let b2 = Math.floor(Date.now() / 1e3);
        return new bs.L({ ...a10 }).setProtectedHeader({ alg: "dir", enc: "A256GCM" }).setIssuedAt(b2).setExpirationTime(b2 + 86400).encrypt(bz);
      }
      async function bD(a10) {
        try {
          let { payload: b2 } = await (0, bt.B)(a10, bz, { clockTolerance: 10 });
          return b2;
        } catch (a11) {
          return null;
        }
      ...
      async function bE(a10) {
        let b2 = a10.headers.get("Cookie") || "", c2 = b2.split(";").find((a11) => a11.trim().startsWith("app_session="))?.split("=")[1];
        return c2 ? bD(c2) : null;
      }
      async function bF(a10, b2 = "portal", c2) {
        let d2 = new URL(a10.url);
        if (process.env.AUTH0_BASE_URL && !d2.searchParams.has("auth_redirect")) try {
          let d3 = new URL(process.env.AUTH0_BASE_URL), e3 = a10.headers, f3 = e3.get("x-forwarded-host") || e3.get("host") || "", g3 = d3.host, h3 = f3.includes("webflow.services");
          if (f3 && !h3 && f3 !== g3) {
            console.log(`[AuthEdge] Smart Redirect. Current: ${f3}, Configured: ${g3}. Redirecting...`);
            let a11 = new URL(`${d3.origin}${bA}/auth/login`);
            return b2 && a11.searchParams.set("type", b2), c2 && a11.searchParams.set("screen_hint", c2), a11.searchParams.set("auth_redirect", "true"), new Response(null, { status: 302, headers: { Location: a11.toString() } });
          }
        } catch (a11) {
          console.error("[AuthEdge] Smart Redirect Check Failed:", a11);
        }
        let e2 = process.env.AUTH0_BASE_URL ? `${new URL(process.env.AUTH0_BASE_URL).origin}${bA}` : bB(a10), f2 = `${e2}/auth/callback`;
        console.log(`[AuthEdge] Login initiated. Redirect URI: ${f2}`);
        let g2 = crypto.randomUUID(), h2 = new URL(`https://${bv}/authorize`);
        h2.searchParams.set("client_id", bw), h2.searchParams.set("response_type", "code"), h2.searchParams.set("scope", "openid profile email"), h2.searchParams.set("redirect_uri", f2), h2.searchParams.set("state", g2), c2 && h2.searchParams.set("screen_hint", c2);
        let i2 = S.redirect(h2);
        return i2.cookies.set("auth0_state", `${g2}:${b2}`, { path: "/", httpOnly: true, secure: true, sameSite: "lax", maxAge: 300 }), i2.cookies.set("app_session", "", { path: "/", httpOnly: true, secure: true, sameSite: "lax", maxAge: 0 }), i2;
      }
      async function bG(a10) {
        let b2 = new URL(a10.url), c2 = b2.searchParams.get("code"), d2 = b2.searchParams.get("state"), e2 = a10.headers.get("Cookie") || "", f2 = e2.split(";").find((a11) => a11.trim().startsWith("auth0_state="))?.split("=")[1], g2 = f2 ? decodeURIComponent(f2) : void 0, h2 = b2.searchParams.get("error"), i2 = b2.searchParams.get("error_description");
        if (h2) return console.error(`[AuthEdge] Auth0 Error: ${h2} - ${i2}`), new Response(null, { status: 302, headers: { Location: `${bB(a10)}/auth/login?error=${encodeURIComponent(h2)}` } });
        if (!c2 || !d2 || !g2) return console.error("[AuthEdge] Missing code/state/cookie. Redirecting to login to restart."), new Response(null, { status: 302, headers: { Location: `${bB(a10)}/auth/login?error=missing_state` } });
        let [j2, k2] = g2.split(":");
        if (d2 !== j2) return console.error(`[AuthEdge] State mismatch. Param: ${d2}, Cookie: ${j2}. Redirecting to login.`), new Response(null, { status: 302, headers: { Location: `${bB(a10)}/auth/login?error=invalid_state` } });
        let l2 = process.env.AUTH0_BASE_URL ? `${new URL(process.env.AUTH0_BASE_URL).origin}${bA}` : bB(a10), m2 = `${l2}/auth/callback`, n2 = await fetch(`https://${bv}/oauth/token`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ grant_type: "authorization_code", client_id: bw, client_secret: bx, code: c2, redirect_uri: m2 }) });
        if (!n2.ok) return console.error("Auth0 Token Exchange Failed:", await n2.text()), new Response(null, { status: 302, headers: { Location: `${bB(a10)}/auth/login?error=token_exchange_failed` } });
        let { id_token: o2 } = await n2.json();
        try {
          let b3 = await bI(o2);
          if (!b3) throw Error("Verification failed");
          let c3 = { auth0Id: b3.sub, email: b3.email, name: b3.name || b3.nickname, picture: b3.picture, flow: "authenticated", loginType: k2 || "portal" }, d3 = await bC(c3), e3 = new Headers();
          return e3.append("Location", `${bB(a10)}/auth/post-login`), e3.append("Set-Cookie", `app_session=${d3}; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=86400`), e3.append("Set-Cookie", "auth0_state=; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=0"), new Response(null, { status: 302, headers: e3 });
        } catch (b3) {
          return console.error("JWT Verification Failed:", b3), new Response(null, { status: 302, headers: { Location: `${bB(a10)}/auth/login?error=invalid_token` } });
        }
      }
      async function bH(a10) {
        let b2 = new URL(`https://${bv}/v2/logout`);
        b2.searchParams.set("client_id", bw);
        let c2 = bB(a10);
        return b2.searchParams.set("returnTo", c2), new Response(null, { status: 302, headers: { Location: b2.toString(), "Set-Cookie": "app_session=; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=0" } });
      }
      async function bI(a10) {
        try {
          let b2 = await Promise.resolve().then(c.bind(c, 4303)).then((a11) => a11.createRemoteJWKSet(new URL(`https://${bv}/.well-known/jwks.json`))), { payload: d2 } = await (0, bu.V)(a10, b2, { issuer: `https://${bv}/`, audience: bw });
          return d2;
        } catch (a11) {
          return console.error("Token verification failed", a11), null;
        }
      }
      async function bJ(a10) {
        let { pathname: b2 } = a10.nextUrl;
        console.log(`[Middleware] Original Path: ${b2}`), b2.startsWith("/app") && (b2 = b2.replace(/^\/app/, "") || "/"), console.log(`[Middleware] Normalized Path: ${b2}`);
        let c2 = (a11) => b2 === a11 || b2 === `${a11}/`;
        if (c2("/auth/login") || c2("/auth/login/portal")) return bF(a10, "portal");
        if (c2("/auth/login/student")) return bF(a10, "student");
        if (c2("/auth/signup")) return S.next();
        if (c2("/auth/callback")) return bG(a10);
        if (c2("/auth/logout")) return bH(a10);
        if (b2.startsWith("/api")) return console.log(`[Middleware] Allowing API route ${b2} to pass through.`), S.next();
        let d2 = await bE(a10);
        if (!d2) return b2.startsWith("/auth/") ? (console.log(`[Middleware] No session on ${b2}, allowing pass-through/native handling`), S.next()) : ("/" === b2 || "/app" === b2 || "/app/" === b2 ? console.log("[Middleware] No session on root, strictly redirecting to login") : console.log("[Middleware] No session, strictly redirecting to login"), S.redirect(new URL("/app/auth/login", a10.url)));
        let e2 = d2.flow || "authenticated";
        switch (console.log(`[Middleware] Session Found. Flow: ${e2}`), e2) {
          case "authenticated":
          case "syncing":
            if (b2.endsWith("/auth/complete-register")) return console.log(`[Middleware] Flow '${e2}' - allowing access to register page (permissive)`), S.next();
            if (!b2.endsWith("/auth/post-login")) {
              console.log(`[Middleware] Flow '${e2}' - blocking access to ${b2}, redirecting to post-login`);
              let c3 = parseInt(a10.nextUrl.searchParams.get("rc") || "0");
              if (c3 > 3) return console.error("[Middleware] Infinite Redirect Loop Detected. Breaking loop."), S.redirect(new URL("/app/auth/login?error=infinite_loop", a10.url));
              let d3 = new URL("/app/auth/post-login", a10.url);
              return d3.searchParams.set("rc", (c3 + 1).toString()), S.redirect(d3);
            }
            return console.log(`[Middleware] Flow '${e2}' - allowing access to post-login page`), S.next();
          case "onboarding_required":
            if (!b2.endsWith("/auth/complete-register")) return console.log(`[Middleware] Flow '${e2}' - blocking access to ${b2}, redirecting to register`), S.redirect(new URL("/app/auth/complete-register", a10.url));
            return console.log(`[Middleware] Flow '${e2}' - creating access to register page`), S.next();
          case "ready":
            if (b2.endsWith("/auth/post-login") || b2.endsWith("/auth/complete-register")) return console.log(`[Middleware] Flow '${e2}' - redirecting to dashboard`), S.redirect(new URL("/app", a10.url));
            return console.log(`[Middleware] Flow '${e2}' - allowing access to ${b2}`), S.next();
          default:
            if ("/" === b2 || "/app" === b2 || "/app/" === b2) return console.log(`[Middleware] Root access with unknown flow '${e2}' - allowing pass through`), S.next();
            return console.log(`[Middleware] Unknown flow '${e2}' - allowing pass through`), S.next();
        }
      }
      let bK = { matcher: ["/((?!_next/static|_next/image|favicon.ico).*)"] };
      Object.values({ NOT_FOUND: 404, FORBIDDEN: 403, UNAUTHORIZED: 401 });
      let bL = { ...f }, bM = bL.middleware || bL.default, bN = "/src/middleware";
      if ("function" != typeof bM) throw Object.defineProperty(Error(`The Middleware "${bN}" must export a \`middleware\` or a \`default\` function`), "__NEXT_ERROR_CODE", { value: "E120", enumerable: false, configurable: true });
      function bO(a10) {
        return bf({ ...a10, page: bN, handler: async (...a11) => {
          try {
            return await bM(...a11);
          } catch (e2) {
      ...
      function d(a2, b2, ...c2) {
        if ((c2 = c2.filter(Boolean)).length > 2) {
          let b3 = c2.pop();
          a2 += `one of type ${c2.join(", ")}, or ${b3}.`;
        } else 2 === c2.length ? a2 += `one of type ${c2[0]} or ${c2[1]}.` : a2 += `of type ${c2[0]}.`;
        return null == b2 ? a2 += ` Received ${b2}` : "function" == typeof b2 && b2.name ? a2 += ` Received function ${b2.name}` : "object" == typeof b2 && null != b2 && b2.constructor?.name && (a2 += ` Received an instance of ${b2.constructor.name}`), a2;
      }
      c.d(b, { X: () => e, t: () => f });
      let e = (a2, ...b2) => d("Key must be ", a2, ...b2), f = (a2, b2, ...c2) => d(`Key for the ${a2} algorithm must be `, b2, ...c2);
    }, 703: (a, b, c) => {
      "use strict";
      c.d(b, { Y: () => i, w: () => h });
      var d = c(9814), e = c(2934), f = c(9663);
      let g = (a2) => {
        switch (a2) {
          case "RSA-OAEP":
          case "RSA-OAEP-256":
          ...
          case "RSA-OAEP-512":
            return "RSA-OAEP";
          default:
            throw new f.JOSENotSupported(`alg ${a2} is not supported either by JOSE or your javascript runtime`);
        }
      };
      async function h(a2, b2, c2) {
        return (0, d.$)(b2, a2, "encrypt"), (0, e.O)(a2, b2), new Uint8Array(await crypto.subtle.encrypt(g(a2), b2, c2));
      }
      async function i(a2, b2, c2) {
        return (0, d.$)(b2, a2, "decrypt"), (0, e.O)(a2, b2), new Uint8Array(await crypto.subtle.decrypt(g(a2), b2, c2));
      }
    }, 798: (a, b, c) => {
      "use strict";
      function d(...a2) {
        let b2, c2 = a2.filter(Boolean);
        if (0 === c2.length || 1 === c2.length) return true;
        for (let a3 of c2) {
          let c3 = Object.keys(a3);
          if (!b2 || 0 === b2.size) {
            b2 = new Set(c3);
            continue;
          }
          for (let a4 of c3) {
            if (b2.has(a4)) return false;
            b2.add(a4);
          }
        }
        return true;
      ...
      "use strict";
      c.d(b, { S: () => f });
      var d = c(9663), e = c(1180);
      function f(a2, b2) {
        if (b2.length << 3 !== (0, e.a)(a2)) throw new d.JWEInvalid("Invalid Initialization Vector length");
      }
    }, 1180: (a, b, c) => {
      "use strict";
      c.d(b, { a: () => e, n: () => f });
      ...
      function e(a2) {
        switch (a2) {
          ...
          case "A256CBC-HS512":
            return 128;
          default:
            throw new d.JOSENotSupported(`Unsupported JWE Algorithm: ${a2}`);
        }
      }
      let f = (a2) => crypto.getRandomValues(new Uint8Array(e(a2) >> 3));
    }, 1213: (a) => {
      (() => {
        "use strict";
        var b = { 993: (a2) => {
      ...
      "use strict";
      function d(a2) {
        if ("object" != typeof a2 || null === a2 || "[object Object]" !== Object.prototype.toString.call(a2)) return false;
        if (null === Object.getPrototypeOf(a2)) return true;
        let b2 = a2;
        for (; null !== Object.getPrototypeOf(b2); ) b2 = Object.getPrototypeOf(b2);
        return Object.getPrototypeOf(a2) === b2;
      }
      c.d(b, { G: () => d });
    }, 2072: (a, b, c) => {
      "use strict";
      c.d(b, { n: () => e });
      var d = c(9663);
      function e(a2, b2, c2, e2, f) {
        let g;
        if (void 0 !== f.crit && e2?.crit === void 0) throw new a2('"crit" (Critical) Header Parameter MUST be integrity protected');
        if (!e2 || void 0 === e2.crit) return /* @__PURE__ */ new Set();
        if (!Array.isArray(e2.crit) || 0 === e2.crit.length || e2.crit.some((a3) => "string" != typeof a3 || 0 === a3.length)) throw new a2('"crit" (Critical) Header Parameter MUST be an array of non-empty strings when present');
        for (let h of (g = void 0 !== c2 ? new Map([...Object.entries(c2), ...b2.entries()]) : b2, e2.crit)) {
          if (!g.has(h)) throw new d.JOSENotSupported(`Extension Header Parameter "${h}" is not recognized`);
          if (void 0 === f[h]) throw new a2(`Extension Header Parameter "${h}" is missing`);
          if (g.get(h) && void 0 === e2[h]) throw new a2(`Extension Header Parameter "${h}" MUST be integrity protected`);
        }
        return new Set(e2.crit);
      }
    }, 2356: (a, b, c) => {
      "use strict";
      c.d(b, { W: () => e });
      var d = c(9663);
      function e(a2, b2) {
        let c2 = a2.byteLength << 3;
        if (c2 !== b2) throw new d.JWEInvalid(`Invalid Content Encryption Key length. Expected ${b2} bits, got ${c2} bits`);
      }
    }, 2877: (a, b, c) => {
      "use strict";
      c.d(b, { x: () => e });
      var d = c(9663);
      async function e(a2) {
        if (!a2.alg) throw TypeError('"alg" argument is required when "jwk.alg" is not present');
        let { algorithm: b2, keyUsages: c2 } = function(a3) {
          let b3, c3;
          switch (a3.kty) {
            case "AKP":
              switch (a3.alg) {
                ...
                case "ML-DSA-65":
                case "ML-DSA-87":
                  b3 = { name: a3.alg }, c3 = a3.priv ? ["sign"] : ["verify"];
                  break;
                default:
                  throw new d.JOSENotSupported('Invalid or unsupported JWK "alg" (Algorithm) Parameter value');
              }
              break;
            case "RSA":
              switch (a3.alg) {
                ...
                case "PS384":
                case "PS512":
                  b3 = { name: "RSA-PSS", hash: `SHA-${a3.alg.slice(-3)}` }, c3 = a3.d ? ["sign"] : ["verify"];
                  break;
                case "RS256":
                case "RS384":
                case "RS512":
                  b3 = { name: "RSASSA-PKCS1-v1_5", hash: `SHA-${a3.alg.slice(-3)}` }, c3 = a3.d ? ["sign"] : ["verify"];
                  break;
                case "RSA-OAEP":
                case "RSA-OAEP-256":
                case "RSA-OAEP-384":
                case "RSA-OAEP-512":
                  b3 = { name: "RSA-OAEP", hash: `SHA-${parseInt(a3.alg.slice(-3), 10) || 1}` }, c3 = a3.d ? ["decrypt", "unwrapKey"] : ["encrypt", "wrapKey"];
                  break;
                default:
                  throw new d.JOSENotSupported('Invalid or unsupported JWK "alg" (Algorithm) Parameter value');
              }
            ...
            case "EC":
              switch (a3.alg) {
                case "ES256":
                  b3 = { name: "ECDSA", namedCurve: "P-256" }, c3 = a3.d ? ["sign"] : ["verify"];
                  break;
                case "ES384":
                  b3 = { name: "ECDSA", namedCurve: "P-384" }, c3 = a3.d ? ["sign"] : ["verify"];
                  break;
                case "ES512":
                  b3 = { name: "ECDSA", namedCurve: "P-521" }, c3 = a3.d ? ["sign"] : ["verify"];
                  break;
                case "ECDH-ES":
                case "ECDH-ES+A128KW":
                case "ECDH-ES+A192KW":
                case "ECDH-ES+A256KW":
                  b3 = { name: "ECDH", namedCurve: a3.crv }, c3 = a3.d ? ["deriveBits"] : [];
                  break;
                default:
                  throw new d.JOSENotSupported('Invalid or unsupported JWK "alg" (Algorithm) Parameter value');
              }
            ...
            case "OKP":
              switch (a3.alg) {
                case "Ed25519":
                case "EdDSA":
                  b3 = { name: "Ed25519" }, c3 = a3.d ? ["sign"] : ["verify"];
                  break;
                case "ECDH-ES":
                case "ECDH-ES+A128KW":
                case "ECDH-ES+A192KW":
                case "ECDH-ES+A256KW":
                  b3 = { name: a3.crv }, c3 = a3.d ? ["deriveBits"] : [];
                  break;
                default:
                  throw new d.JOSENotSupported('Invalid or unsupported JWK "alg" (Algorithm) Parameter value');
              }
              break;
            default:
              throw new d.JOSENotSupported('Invalid or unsupported JWK "kty" (Key Type) Parameter value');
          }
          return { algorithm: b3, keyUsages: c3 };
        }(a2), e2 = { ...a2 };
        return "AKP" !== e2.kty && delete e2.alg, delete e2.use, crypto.subtle.importKey("jwk", e2, b2, a2.ext ?? (!a2.d && !a2.priv), a2.key_ops ?? c2);
      }
    }, 2894: (a, b, c) => {
      "use strict";
      c.d(b, { w: () => n });
      var d = c(8629), e = c(1096), f = c(2356), g = c(9814), h = c(269), i = c(1180), j = c(9663), k = c(6495);
      async function l(a2, b2, c2, e2, f2) {
        if (!(c2 instanceof Uint8Array)) throw TypeError((0, h.X)(c2, "Uint8Array"));
        let g2 = parseInt(a2.slice(1, 4), 10), i2 = await crypto.subtle.importKey("raw", c2.subarray(g2 >> 3), "AES-CBC", false, ["encrypt"]), j2 = await crypto.subtle.importKey("raw", c2.subarray(0, g2 >> 3), { hash: `SHA-${g2 << 1}`, name: "HMAC" }, false, ["sign"]), k2 = new Uint8Array(await crypto.subtle.encrypt({ iv: e2, name: "AES-CBC" }, i2, b2)), l2 = (0, d.xW)(f2, e2, k2, (0, d.mx)(f2.length << 3));
        return { ciphertext: k2, tag: new Uint8Array((await crypto.subtle.sign("HMAC", j2, l2)).slice(0, g2 >> 3)), iv: e2 };
      }
      async function m(a2, b2, c2, d2, e2) {
        let f2;
        c2 instanceof Uint8Array ? f2 = await crypto.subtle.importKey("raw", c2, "AES-GCM", false, ["encrypt"]) : ((0, g.$)(c2, a2, "encrypt"), f2 = c2);
        let h2 = new Uint8Array(await crypto.subtle.encrypt({ additionalData: e2, iv: d2, name: "AES-GCM", tagLength: 128 }, f2, b2)), i2 = h2.slice(-16);
        return { ciphertext: h2.slice(0, -16), tag: i2, iv: d2 };
      }
      async function n(a2, b2, c2, d2, g2) {
        if (!(0, k.R7)(c2) && !(c2 instanceof Uint8Array)) throw TypeError((0, h.X)(c2, "CryptoKey", "KeyObject", "Uint8Array", "JSON Web Key"));
        switch (d2 ? (0, e.S)(a2, d2) : d2 = (0, i.n)(a2), a2) {
          case "A128CBC-HS256":
          case "A192CBC-HS384":
          case "A256CBC-HS512":
            return c2 instanceof Uint8Array && (0, f.W)(c2, parseInt(a2.slice(-3), 10)), l(a2, b2, c2, d2, g2);
          case "A128GCM":
          case "A192GCM":
          case "A256GCM":
            return c2 instanceof Uint8Array && (0, f.W)(c2, parseInt(a2.slice(1, 4), 10)), m(a2, b2, c2, d2, g2);
          default:
            throw new j.JOSENotSupported("Unsupported JWE Content Encryption Algorithm");
        }
      }
    }, 2934: (a, b, c) => {
      "use strict";
      function d(a2, b2) {
        if (a2.startsWith("RS") || a2.startsWith("PS")) {
          let { modulusLength: c2 } = b2.algorithm;
          if ("number" != typeof c2 || c2 < 2048) throw TypeError(`${a2} requires key modulusLength to be 2048 bits or larger`);
        }
      }
      c.d(b, { O: () => d });
    }, 3169: (a, b, c) => {
      "use strict";
      c.d(b, { cE: () => m, k6: () => l });
      var d = c(9663), e = c(8629), f = c(1967);
      let g = (a2) => Math.floor(a2.getTime() / 1e3), h = /^(\+|\-)? ?(\d+|\d+\.\d+) ?(seconds?|secs?|s|minutes?|mins?|m|hours?|hrs?|h|days?|d|weeks?|w|years?|yrs?|y)(?: (ago|from now))?$/i;
      function i(a2) {
        let b2, c2 = h.exec(a2);
      ...
      function j(a2, b2) {
        if (!Number.isFinite(b2)) throw TypeError(`Invalid ${a2} input`);
        return b2;
      }
      let k = (a2) => a2.includes("/") ? a2.toLowerCase() : `application/${a2.toLowerCase()}`;
      function l(a2, b2, c2 = {}) {
        var h2, j2;
        let m2, n;
        try {
          m2 = JSON.parse(e.D0.decode(b2));
        } catch {
        }
        if (!(0, f.G)(m2)) throw new d.JWTInvalid("JWT Claims Set must be a top-level JSON object");
        let { typ: o } = c2;
        if (o && ("string" != typeof a2.typ || k(a2.typ) !== k(o))) throw new d.JWTClaimValidationFailed('unexpected "typ" JWT header value', m2, "typ", "check_failed");
        let { requiredClaims: p = [], issuer: q, subject: r, audience: s, maxTokenAge: t } = c2, u = [...p];
        for (let a3 of (void 0 !== t && u.push("iat"), void 0 !== s && u.push("aud"), void 0 !== r && u.push("sub"), void 0 !== q && u.push("iss"), new Set(u.reverse()))) if (!(a3 in m2)) throw new d.JWTClaimValidationFailed(`missing required "${a3}" claim`, m2, a3, "missing");
        if (q && !(Array.isArray(q) ? q : [q]).includes(m2.iss)) throw new d.JWTClaimValidationFailed('unexpected "iss" claim value', m2, "iss", "check_failed");
        if (r && m2.sub !== r) throw new d.JWTClaimValidationFailed('unexpected "sub" claim value', m2, "sub", "check_failed");
        if (s && (h2 = m2.aud, j2 = "string" == typeof s ? [s] : s, "string" == typeof h2 ? !j2.includes(h2) : !(Array.isArray(h2) && j2.some(Set.prototype.has.bind(new Set(h2)))))) throw new d.JWTClaimValidationFailed('unexpected "aud" claim value', m2, "aud", "check_failed");
        switch (typeof c2.clockTolerance) {
          case "string":
            n = i(c2.clockTolerance);
            break;
          case "number":
            n = c2.clockTolerance;
            break;
        ...
        let { currentDate: v } = c2, w = g(v || /* @__PURE__ */ new Date());
        if ((void 0 !== m2.iat || t) && "number" != typeof m2.iat) throw new d.JWTClaimValidationFailed('"iat" claim must be a number', m2, "iat", "invalid");
        if (void 0 !== m2.nbf) {
          if ("number" != typeof m2.nbf) throw new d.JWTClaimValidationFailed('"nbf" claim must be a number', m2, "nbf", "invalid");
          if (m2.nbf > w + n) throw new d.JWTClaimValidationFailed('"nbf" claim timestamp check failed', m2, "nbf", "check_failed");
        }
        if (void 0 !== m2.exp) {
          if ("number" != typeof m2.exp) throw new d.JWTClaimValidationFailed('"exp" claim must be a number', m2, "exp", "invalid");
          if (m2.exp <= w - n) throw new d.JWTExpired('"exp" claim timestamp check failed', m2, "exp", "check_failed");
        }
        if (t) {
          let a3 = w - m2.iat;
          if (a3 - n > ("number" == typeof t ? t : i(t))) throw new d.JWTExpired('"iat" claim timestamp check failed (too far in the past)', m2, "iat", "check_failed");
          if (a3 < 0 - n) throw new d.JWTClaimValidationFailed('"iat" claim timestamp check failed (it should be in the past)', m2, "iat", "check_failed");
        }
        return m2;
      }
      class m {
        #a;
        constructor(a2) {
          if (!(0, f.G)(a2)) throw TypeError("JWT Claims Set MUST be an object");
          this.#a = structuredClone(a2);
        }
        data() {
          return e.Rd.encode(JSON.stringify(this.#a));
        }
        get iss() {
          return this.#a.iss;
        }
        ...
        set nbf(a2) {
          "number" == typeof a2 ? this.#a.nbf = j("setNotBefore", a2) : a2 instanceof Date ? this.#a.nbf = j("setNotBefore", g(a2)) : this.#a.nbf = g(/* @__PURE__ */ new Date()) + i(a2);
        }
        set exp(a2) {
          "number" == typeof a2 ? this.#a.exp = j("setExpirationTime", a2) : a2 instanceof Date ? this.#a.exp = j("setExpirationTime", g(a2)) : this.#a.exp = g(/* @__PURE__ */ new Date()) + i(a2);
        }
        set iat(a2) {
          void 0 === a2 ? this.#a.iat = g(/* @__PURE__ */ new Date()) : a2 instanceof Date ? this.#a.iat = j("setIssuedAt", g(a2)) : "string" == typeof a2 ? this.#a.iat = j("setIssuedAt", g(/* @__PURE__ */ new Date()) + i(a2)) : this.#a.iat = j("setIssuedAt", a2);
        }
      }
    }, 3229: (a, b, c) => {
      "use strict";
      c.d(b, { j: () => n });
      var d = c(9148), e = c(7622), f = c(2894), g = c(9118), h = c(9663), i = c(798), j = c(8629), k = c(2072), l = c(6144), m = c(3910);
      class n {
        #b;
        #c;
        #d;
        ...
        #i;
        constructor(a2) {
          if (!(a2 instanceof Uint8Array)) throw TypeError("plaintext must be an instance of Uint8Array");
          this.#b = a2;
        }
        setKeyManagementParameters(a2) {
          if (this.#i) throw TypeError("setKeyManagementParameters can only be called once");
          return this.#i = a2, this;
        }
        setProtectedHeader(a2) {
          if (this.#c) throw TypeError("setProtectedHeader can only be called once");
          return this.#c = a2, this;
        }
        setSharedUnprotectedHeader(a2) {
          if (this.#d) throw TypeError("setSharedUnprotectedHeader can only be called once");
          return this.#d = a2, this;
        }
        setUnprotectedHeader(a2) {
          if (this.#e) throw TypeError("setUnprotectedHeader can only be called once");
          return this.#e = a2, this;
        }
        setAdditionalAuthenticatedData(a2) {
          return this.#f = a2, this;
        }
        setContentEncryptionKey(a2) {
          if (this.#g) throw TypeError("setContentEncryptionKey can only be called once");
          return this.#g = a2, this;
        }
        setInitializationVector(a2) {
          if (this.#h) throw TypeError("setInitializationVector can only be called once");
          return this.#h = a2, this;
        }
        async encrypt(a2, b2) {
          let c2, n2, o, p, q, r;
          if (!this.#c && !this.#e && !this.#d) throw new h.JWEInvalid("either setProtectedHeader, setUnprotectedHeader, or sharedUnprotectedHeader must be called before #encrypt()");
          if (!(0, i.f)(this.#c, this.#e, this.#d)) throw new h.JWEInvalid("JWE Protected, JWE Shared Unprotected and JWE Per-Recipient Header Parameter names must be disjoint");
          let s = { ...this.#c, ...this.#e, ...this.#d };
          if ((0, k.n)(h.JWEInvalid, /* @__PURE__ */ new Map(), b2?.crit, this.#c, s), void 0 !== s.zip) throw new h.JOSENotSupported('JWE "zip" (Compression Algorithm) Header Parameter is not supported.');
          let { alg: t, enc: u } = s;
          if ("string" != typeof t || !t) throw new h.JWEInvalid('JWE "alg" (Algorithm) Header Parameter missing or invalid');
          if ("string" != typeof u || !u) throw new h.JWEInvalid('JWE "enc" (Encryption Algorithm) Header Parameter missing or invalid');
          if (this.#g && ("dir" === t || "ECDH-ES" === t)) throw TypeError(`setContentEncryptionKey cannot be called with JWE "alg" (Algorithm) Header ${t}`);
          (0, m.y)("dir" === t ? u : t, a2, "encrypt");
          {
            let d2, f2 = await (0, l.l)(a2, t);
            ({ cek: n2, encryptedKey: c2, parameters: d2 } = await (0, g.s)(t, u, f2, this.#g, this.#i)), d2 && (b2 && e.o in b2 ? this.#e ? this.#e = { ...this.#e, ...d2 } : this.setUnprotectedHeader(d2) : this.#c ? this.#c = { ...this.#c, ...d2 } : this.setProtectedHeader(d2));
          }
          if (this.#c ? (p = (0, d.encode)(JSON.stringify(this.#c)), q = (0, j.lF)(p)) : (p = "", q = new Uint8Array()), this.#f) {
            r = (0, d.encode)(this.#f);
            let a3 = (0, j.lF)(r);
            o = (0, j.xW)(q, (0, j.lF)("."), a3);
          } else o = q;
          let { ciphertext: v, tag: w, iv: x } = await (0, f.w)(u, this.#b, n2, this.#h, o), y = { ciphertext: (0, d.encode)(v) };
          return x && (y.iv = (0, d.encode)(x)), w && (y.tag = (0, d.encode)(w)), c2 && (y.encrypted_key = (0, d.encode)(c2)), r && (y.aad = r), this.#c && (y.protected = p), this.#d && (y.unprotected = this.#d), this.#e && (y.header = this.#e), y;
        }
      }
    }, 3340: (a, b, c) => {
      "use strict";
      c.d(b, { JU: () => k, Yp: () => j, Y5: () => i });
      var d = c(8774), e = c(269), f = c(9148), g = c(6495);
      async function h(a2) {
        if ((0, g.D2)(a2)) if ("secret" !== a2.type) return a2.export({ format: "jwk" });
        else a2 = a2.export();
        if (a2 instanceof Uint8Array) return { kty: "oct", k: (0, f.encode)(a2) };
        if (!(0, g.R7)(a2)) throw TypeError((0, e.X)(a2, "CryptoKey", "KeyObject", "Uint8Array"));
        if (!a2.extractable) throw TypeError("non-extractable CryptoKey cannot be exported as a JWK");
        let { ext: b2, key_ops: c2, alg: d2, use: h2, ...i2 } = await crypto.subtle.exportKey("jwk", a2);
        return "AKP" === i2.kty && (i2.alg = d2), i2;
      }
      async function i(a2) {
        return (0, d.Nj)(a2);
      }
      ...
      var d = c(1967);
      let e = (a2) => (0, d.G)(a2) && "string" == typeof a2.kty, f = (a2) => "oct" !== a2.kty && ("AKP" === a2.kty && "string" == typeof a2.priv || "string" == typeof a2.d), g = (a2) => "oct" !== a2.kty && void 0 === a2.d && void 0 === a2.priv, h = (a2) => "oct" === a2.kty && "string" == typeof a2.k;
    }, 3817: (a, b, c) => {
      (() => {
        "use strict";
        var b2 = { 491: (a2, b3, c2) => {
      ...
      "use strict";
      c.d(b, { y: () => i });
      var d = c(269), e = c(6495), f = c(3652);
      let g = (a2) => a2?.[Symbol.toStringTag], h = (a2, b2, c2) => {
        if (void 0 !== b2.use) {
          let a3;
          switch (c2) {
            case "sign":
          ...
          if (b2.use !== a3) throw TypeError(`Invalid key for this operation, its "use" must be "${a3}" when present`);
        }
        if (void 0 !== b2.alg && b2.alg !== a2) throw TypeError(`Invalid key for this operation, its "alg" must be "${a2}" when present`);
        if (Array.isArray(b2.key_ops)) {
          let d2;
          switch (true) {
            case ("sign" === c2 || "verify" === c2):
            case "dir" === a2:
            case a2.includes("CBC-HS"):
              d2 = c2;
              break;
            case a2.startsWith("PBES2"):
              d2 = "deriveBits";
              break;
            case /^A\d{3}(?:GCM)?(?:KW)?$/.test(a2):
              d2 = !a2.includes("GCM") && a2.endsWith("KW") ? "encrypt" === c2 ? "wrapKey" : "unwrapKey" : c2;
              break;
            case ("encrypt" === c2 && a2.startsWith("RSA")):
              d2 = "wrapKey";
              break;
            case "decrypt" === c2:
              d2 = a2.startsWith("RSA") ? "unwrapKey" : "deriveBits";
          }
          ...
          case "PB":
            ((a3, b3, c3) => {
              if (!(b3 instanceof Uint8Array)) {
                ...
                if (!(0, e.P2)(b3)) throw TypeError((0, d.t)(a3, b3, "CryptoKey", "KeyObject", "JSON Web Key", "Uint8Array"));
                if ("secret" !== b3.type) throw TypeError(`${g(b3)} instances for symmetric algorithms must be of type "secret"`);
              }
            })(a2, b2, c2);
            break;
          default:
            ((a3, b3, c3) => {
              if (f.ll(b3)) switch (c3) {
                case "decrypt":
                case "sign":
                  if (f.W2(b3) && h(a3, b3, c3)) return;
                  throw TypeError("JSON Web Key for this operation must be a private JWK");
                case "encrypt":
                case "verify":
                  if (f.M3(b3) && h(a3, b3, c3)) return;
                  throw TypeError("JSON Web Key for this operation must be a public JWK");
              }
              if (!(0, e.P2)(b3)) throw TypeError((0, d.t)(a3, b3, "CryptoKey", "KeyObject", "JSON Web Key"));
              if ("secret" === b3.type) throw TypeError(`${g(b3)} instances for asymmetric algorithms must not be of type "secret"`);
              if ("public" === b3.type) switch (c3) {
                case "sign":
                  throw TypeError(`${g(b3)} instances for asymmetric algorithm signing must be of type "private"`);
                case "decrypt":
                  throw TypeError(`${g(b3)} instances for asymmetric algorithm decryption must be of type "private"`);
              }
              if ("private" === b3.type) switch (c3) {
                case "verify":
                  throw TypeError(`${g(b3)} instances for asymmetric algorithm verifying must be of type "public"`);
                case "encrypt":
                  throw TypeError(`${g(b3)} instances for asymmetric algorithm encryption must be of type "public"`);
              }
            })(a2, b2, c2);
        }
      }
      ...
      "use strict";
      function d(a2, b2) {
        if (void 0 !== b2 && (!Array.isArray(b2) || b2.some((a3) => "string" != typeof a3))) throw TypeError(`"${a2}" option must be an array of strings`);
      ...
      "use strict";
      let d;
      c.r(b), c.d(b, { CompactEncrypt: () => z.j, CompactSign: () => G, EmbeddedJWK: () => V, EncryptJWT: () => L.L, FlattenedEncrypt: () => j.j, FlattenedSign: () => F, GeneralEncrypt: () => t, GeneralSign: () => I, SignJWT: () => K, UnsecuredJWT: () => ad, base64url: () => o, calculateJwkThumbprint: () => S, calculateJwkThumbprintUri: () => T, compactDecrypt: () => e.d, compactVerify: () => u.B, createLocalJWKSet: () => Z, createRemoteJWKSet: () => ac, cryptoRuntime: () => aj, customFetch: () => $, decodeJwt: () => af, decodeProtectedHeader: () => ae, errors: () => g, exportJWK: () => P.JU, exportPKCS8: () => P.Yp, exportSPKI: () => P.Y5, flattenedDecrypt: () => f.d, flattenedVerify: () => v.Z, generalDecrypt: () => i, generalVerify: () => w, generateKeyPair: () => ah, generateSecret: () => ai, importJWK: () => U.Og, importPKCS8: () => U.Lf, importSPKI: () => U.jU, importX509: () => U.Mh, jwksCache: () => aa, jwtDecrypt: () => y.B, jwtVerify: () => x.V });
      var e = c(6063), f = c(9578), g = c(9663), h = c(1967);
      async function i(a2, b2, c2) {
        if (!(0, h.G)(a2)) throw new g.JWEInvalid("General JWE must be an object");
        if (!Array.isArray(a2.recipients) || !a2.recipients.every(h.G)) throw new g.JWEInvalid("JWE Recipients missing or incorrect type");
        if (!a2.recipients.length) throw new g.JWEInvalid("JWE Recipients has no members");
        for (let d2 of a2.recipients) try {
          return await (0, f.d)({ aad: a2.aad, ciphertext: a2.ciphertext, encrypted_key: d2.encrypted_key, header: d2.header, iv: a2.iv, protected: a2.protected, tag: a2.tag, unprotected: a2.unprotected }, b2, c2);
        } catch {
        }
        throw new g.JWEDecryptionFailed();
      }
      var j = c(3229), k = c(7622), l = c(9074), m = c(798), n = c(9118), o = c(9148), p = c(2072), q = c(6144), r = c(3910);
      class s {
        #j;
        unprotectedHeader;
        ...
        constructor(a2, b2, c2) {
          this.#j = a2, this.key = b2, this.options = c2;
        }
        setUnprotectedHeader(a2) {
          if (this.unprotectedHeader) throw TypeError("setUnprotectedHeader can only be called once");
          return this.unprotectedHeader = a2, this;
        }
        setKeyManagementParameters(a2) {
          if (this.keyManagementParameters) throw TypeError("setKeyManagementParameters can only be called once");
          return this.keyManagementParameters = a2, this;
        }
        addRecipient(...a2) {
          return this.#j.addRecipient(...a2);
        }
        encrypt(...a2) {
          return this.#j.encrypt(...a2);
        }
        done() {
          return this.#j;
        }
      ...
      class t {
        ...
        addRecipient(a2, b2) {
          let c2 = new s(this, a2, { crit: b2?.crit });
          return this.#k.push(c2), c2;
        }
        setProtectedHeader(a2) {
          if (this.#c) throw TypeError("setProtectedHeader can only be called once");
          return this.#c = a2, this;
        ...
        setSharedUnprotectedHeader(a2) {
          if (this.#e) throw TypeError("setSharedUnprotectedHeader can only be called once");
          return this.#e = a2, this;
        }
        setAdditionalAuthenticatedData(a2) {
          return this.#f = a2, this;
        ...
        async encrypt() {
          let a2;
          if (!this.#k.length) throw new g.JWEInvalid("at least one recipient must be added");
          if (1 === this.#k.length) {
            let [a3] = this.#k, b3 = await new j.j(this.#b).setAdditionalAuthenticatedData(this.#f).setProtectedHeader(this.#c).setSharedUnprotectedHeader(this.#e).setUnprotectedHeader(a3.unprotectedHeader).encrypt(a3.key, { ...a3.options }), c3 = { ciphertext: b3.ciphertext, iv: b3.iv, recipients: [{}], tag: b3.tag };
          ...
          for (let b3 = 0; b3 < this.#k.length; b3++) {
            let c3 = this.#k[b3];
            if (!(0, m.f)(this.#c, this.#e, c3.unprotectedHeader)) throw new g.JWEInvalid("JWE Protected, JWE Shared Unprotected and JWE Per-Recipient Header Parameter names must be disjoint");
            let d2 = { ...this.#c, ...this.#e, ...c3.unprotectedHeader }, { alg: e2 } = d2;
            if ("string" != typeof e2 || !e2) throw new g.JWEInvalid('JWE "alg" (Algorithm) Header Parameter missing or invalid');
            if ("dir" === e2 || "ECDH-ES" === e2) throw new g.JWEInvalid('"dir" and "ECDH-ES" alg may only be used with a single recipient');
            if ("string" != typeof d2.enc || !d2.enc) throw new g.JWEInvalid('JWE "enc" (Encryption Algorithm) Header Parameter missing or invalid');
            if (a2) {
              if (a2 !== d2.enc) throw new g.JWEInvalid('JWE "enc" (Encryption Algorithm) Header Parameter must be the same for all recipients');
            } else a2 = d2.enc;
            if ((0, p.n)(g.JWEInvalid, /* @__PURE__ */ new Map(), c3.options.crit, this.#c, d2), void 0 !== d2.zip) throw new g.JOSENotSupported('JWE "zip" (Compression Algorithm) Header Parameter is not supported.');
          }
          let b2 = (0, l.D)(a2), c2 = { ciphertext: "", recipients: [] };
          for (let d2 = 0; d2 < this.#k.length; d2++) {
            let e2 = this.#k[d2], f2 = {};
            if (c2.recipients.push(f2), 0 === d2) {
              let a3 = await new j.j(this.#b).setAdditionalAuthenticatedData(this.#f).setContentEncryptionKey(b2).setProtectedHeader(this.#c).setSharedUnprotectedHeader(this.#e).setUnprotectedHeader(e2.unprotectedHeader).setKeyManagementParameters(e2.keyManagementParameters).encrypt(e2.key, { ...e2.options, [k.o]: true });
              c2.ciphertext = a3.ciphertext, c2.iv = a3.iv, c2.tag = a3.tag, a3.aad && (c2.aad = a3.aad), a3.protected && (c2.protected = a3.protected), a3.unprotected && (c2.unprotected = a3.unprotected), f2.encrypted_key = a3.encrypted_key, a3.header && (f2.header = a3.header);
              continue;
            }
            let g2 = e2.unprotectedHeader?.alg || this.#c?.alg || this.#e?.alg;
            (0, r.y)("dir" === g2 ? a2 : g2, e2.key, "encrypt");
            let h2 = await (0, q.l)(e2.key, g2), { encryptedKey: i2, parameters: l2 } = await (0, n.s)(g2, a2, h2, b2, e2.keyManagementParameters);
            f2.encrypted_key = (0, o.encode)(i2), (e2.unprotectedHeader || l2) && (f2.header = { ...e2.unprotectedHeader, ...l2 });
          }
          return c2;
        }
      }
      var u = c(9035), v = c(6218);
      async function w(a2, b2, c2) {
        if (!(0, h.G)(a2)) throw new g.JWSInvalid("General JWS must be an object");
        if (!Array.isArray(a2.signatures) || !a2.signatures.every(h.G)) throw new g.JWSInvalid("JWS Signatures missing or incorrect type");
        for (let d2 of a2.signatures) try {
          return await (0, v.Z)({ header: d2.header, payload: a2.payload, protected: d2.protected, signature: d2.signature }, b2, c2);
        } catch {
        }
        throw new g.JWSSignatureVerificationFailed();
      }
      var x = c(7276), y = c(4968), z = c(7463), A = c(8767), B = c(2934), C = c(6805);
      async function D(a2, b2, c2) {
        let d2 = await (0, C.j)(a2, b2, "sign");
        return (0, B.O)(a2, d2), new Uint8Array(await crypto.subtle.sign((0, A.B)(a2, d2.algorithm), d2, c2));
      }
      var E = c(8629);
      class F {
        #a;
        ...
        #e;
        constructor(a2) {
          if (!(a2 instanceof Uint8Array)) throw TypeError("payload must be an instance of Uint8Array");
          this.#a = a2;
        }
        setProtectedHeader(a2) {
          if (this.#c) throw TypeError("setProtectedHeader can only be called once");
        ...
        async sign(a2, b2) {
          let c2, d2, e2, f2;
          if (!this.#c && !this.#e) throw new g.JWSInvalid("either setProtectedHeader or setUnprotectedHeader must be called before #sign()");
          if (!(0, m.f)(this.#c, this.#e)) throw new g.JWSInvalid("JWS Protected and JWS Unprotected Header Parameter names must be disjoint");
          let h2 = { ...this.#c, ...this.#e }, i2 = (0, p.n)(g.JWSInvalid, /* @__PURE__ */ new Map([["b64", true]]), b2?.crit, this.#c, h2), j2 = true;
          if (i2.has("b64") && "boolean" != typeof (j2 = this.#c.b64)) throw new g.JWSInvalid('The "b64" (base64url-encode payload) Header Parameter must be a boolean');
          let { alg: k2 } = h2;
          if ("string" != typeof k2 || !k2) throw new g.JWSInvalid('JWS "alg" (Algorithm) Header Parameter missing or invalid');
          (0, r.y)(k2, a2, "sign"), j2 ? (c2 = (0, o.encode)(this.#a), d2 = (0, E.lF)(c2)) : (d2 = this.#a, c2 = ""), this.#c ? (e2 = (0, o.encode)(JSON.stringify(this.#c)), f2 = (0, E.lF)(e2)) : (e2 = "", f2 = new Uint8Array());
          let l2 = (0, E.xW)(f2, (0, E.lF)("."), d2), n2 = await (0, q.l)(a2, k2), s2 = await D(k2, n2, l2), t2 = { signature: (0, o.encode)(s2), payload: c2 };
          return this.#e && (t2.header = this.#e), this.#c && (t2.protected = e2), t2;
        }
      }
      class G {
        #l;
        ...
        setProtectedHeader(a2) {
          return this.#l.setProtectedHeader(a2), this;
        }
        async sign(a2, b2) {
          let c2 = await this.#l.sign(a2, b2);
      ...
      class H {
        ...
        setProtectedHeader(a2) {
          ...
          return this.protectedHeader = a2, this;
        }
        setUnprotectedHeader(a2) {
          if (this.unprotectedHeader) throw TypeError("setUnprotectedHeader can only be called once");
          return this.unprotectedHeader = a2, this;
        ...
        addSignature(...a2) {
          return this.#j.addSignature(...a2);
        }
        sign(...a2) {
          return this.#j.sign(...a2);
        }
        done() {
          return this.#j;
        }
      ...
      class I {
        ...
        addSignature(a2, b2) {
          let c2 = new H(this, a2, b2);
          return this.#m.push(c2), c2;
        }
        async sign() {
          if (!this.#m.length) throw new g.JWSInvalid("at least one signature must be added");
          let a2 = { signatures: [], payload: "" };
          for (let b2 = 0; b2 < this.#m.length; b2++) {
            let c2 = this.#m[b2], d2 = new F(this.#a);
            ...
            if (0 === b2) a2.payload = e2;
            else if (a2.payload !== e2) throw new g.JWSInvalid("inconsistent use of JWS Unencoded Payload (RFC7797)");
            a2.signatures.push(f2);
          }
          return a2;
        }
      }
      ...
      class K {
        ...
        setIssuer(a2) {
          return this.#n.iss = a2, this;
        }
        setSubject(a2) {
          return this.#n.sub = a2, this;
        }
        setAudience(a2) {
          return this.#n.aud = a2, this;
        }
        setJti(a2) {
          return this.#n.jti = a2, this;
        }
        setNotBefore(a2) {
          return this.#n.nbf = a2, this;
        }
        setExpirationTime(a2) {
          return this.#n.exp = a2, this;
        }
        setIssuedAt(a2) {
          return this.#n.iat = a2, this;
        }
        setProtectedHeader(a2) {
          return this.#c = a2, this;
        }
        async sign(a2, b2) {
          let c2 = new G(this.#n.data());
          if (c2.setProtectedHeader(this.#c), Array.isArray(this.#c?.crit) && this.#c.crit.includes("b64") && false === this.#c.b64) throw new g.JWTInvalid("JWTs MUST NOT use unencoded payload");
          return c2.sign(a2, b2);
        }
      }
      var L = c(5124), M = c(5461), N = c(6495), O = c(3652), P = c(3340), Q = c(269);
      let R = (a2, b2) => {
        if ("string" != typeof a2 || !a2) throw new g.JWKInvalid(`${b2} missing or invalid`);
      };
      async function S(a2, b2) {
        let c2, d2;
        if ((0, O.ll)(a2)) c2 = a2;
        else if ((0, N.P2)(a2)) c2 = await (0, P.JU)(a2);
        else throw TypeError((0, Q.X)(a2, "CryptoKey", "KeyObject", "JSON Web Key"));
        if ("sha256" !== (b2 ??= "sha256") && "sha384" !== b2 && "sha512" !== b2) throw TypeError('digestAlgorithm must one of "sha256", "sha384", or "sha512"');
        switch (c2.kty) {
          case "AKP":
            R(c2.alg, '"alg" (Algorithm) Parameter'), R(c2.pub, '"pub" (Public key) Parameter'), d2 = { alg: c2.alg, kty: c2.kty, pub: c2.pub };
            break;
          case "EC":
            R(c2.crv, '"crv" (Curve) Parameter'), R(c2.x, '"x" (X Coordinate) Parameter'), R(c2.y, '"y" (Y Coordinate) Parameter'), d2 = { crv: c2.crv, kty: c2.kty, x: c2.x, y: c2.y };
            break;
          case "OKP":
            R(c2.crv, '"crv" (Subtype of Key Pair) Parameter'), R(c2.x, '"x" (Public Key) Parameter'), d2 = { crv: c2.crv, kty: c2.kty, x: c2.x };
            break;
          case "RSA":
            R(c2.e, '"e" (Exponent) Parameter'), R(c2.n, '"n" (Modulus) Parameter'), d2 = { e: c2.e, kty: c2.kty, n: c2.n };
            break;
          case "oct":
            R(c2.k, '"k" (Key Value) Parameter'), d2 = { k: c2.k, kty: c2.kty };
            break;
          default:
            throw new g.JOSENotSupported('"kty" (Key Type) Parameter missing or unsupported');
        }
        let e2 = (0, E.lF)(JSON.stringify(d2));
        return (0, o.encode)(await (0, M.b)(b2, e2));
      }
      async function T(a2, b2) {
        b2 ??= "sha256";
        let c2 = await S(a2, b2);
        return `urn:ietf:params:oauth:jwk-thumbprint:sha-${b2.slice(-3)}:${c2}`;
      }
      var U = c(194);
      async function V(a2, b2) {
        let c2 = { ...a2, ...b2?.header };
        if (!(0, h.G)(c2.jwk)) throw new g.JWSInvalid('"jwk" (JSON Web Key) Header Parameter must be a JSON object');
        let d2 = await (0, U.Og)({ ...c2.jwk, ext: true }, c2.alg);
        if (d2 instanceof Uint8Array || "public" !== d2.type) throw new g.JWSInvalid('"jwk" (JSON Web Key) Header Parameter must be a public key');
        return d2;
      }
      function W(a2) {
        return (0, h.G)(a2);
      ...
      class X {
        ...
        constructor(a2) {
          if (!function(a3) {
            return a3 && "object" == typeof a3 && Array.isArray(a3.keys) && a3.keys.every(W);
          }(a2)) throw new g.JWKSInvalid("JSON Web Key Set malformed");
          this.#o = structuredClone(a2);
        }
        jwks() {
          return this.#o;
        }
        async getKey(a2, b2) {
          let { alg: c2, kid: d2 } = { ...a2, ...b2?.header }, e2 = function(a3) {
            switch ("string" == typeof a3 && a3.slice(0, 2)) {
              case "RS":
              case "PS":
                return "RSA";
              case "ES":
              ...
              case "ML":
                return "AKP";
              default:
                throw new g.JOSENotSupported('Unsupported "alg" value for a JSON Web Key Set');
            }
          }(c2), f2 = this.#o.keys.filter((a3) => {
            let b3 = e2 === a3.kty;
            if (b3 && "string" == typeof d2 && (b3 = d2 === a3.kid), b3 && ("string" == typeof a3.alg || "AKP" === e2) && (b3 = c2 === a3.alg), b3 && "string" == typeof a3.use && (b3 = "sig" === a3.use), b3 && Array.isArray(a3.key_ops) && (b3 = a3.key_ops.includes("verify")), b3) switch (c2) {
              case "ES256":
                b3 = "P-256" === a3.crv;
                break;
              case "ES384":
          ...
          if (0 === i2) throw new g.JWKSNoMatchingKey();
          if (1 !== i2) {
            let a3 = new g.JWKSMultipleMatchingKeys(), b3 = this.#p;
            throw a3[Symbol.asyncIterator] = async function* () {
              for (let a4 of f2) try {
                yield await Y(b3, a4, c2);
              } catch {
              }
            }, a3;
          }
          return Y(this.#p, h2, c2);
        }
      }
      async function Y(a2, b2, c2) {
        let d2 = a2.get(b2) || a2.set(b2, {}).get(b2);
        if (void 0 === d2[c2]) {
          let a3 = await (0, U.Og)({ ...b2, ext: true }, c2);
          if (a3 instanceof Uint8Array || "public" !== a3.type) throw new g.JWKSInvalid("JSON Web Key Set members must be public keys");
          d2[c2] = a3;
        }
        return d2[c2];
      }
      function Z(a2) {
        let b2 = new X(a2), c2 = async (a3, c3) => b2.getKey(a3, c3);
        return Object.defineProperties(c2, { jwks: { value: () => structuredClone(b2.jwks()), enumerable: false, configurable: false, writable: false } }), c2;
      }
      "undefined" != typeof navigator && navigator.userAgent?.startsWith?.("Mozilla/5.0 ") || (d = "jose/v6.1.3");
      let $ = Symbol();
      async function _(a2, b2, c2, d2 = fetch) {
        let e2 = await d2(a2, { method: "GET", signal: c2, redirect: "manual", headers: b2 }).catch((a3) => {
          if ("TimeoutError" === a3.name) throw new g.JWKSTimeout();
          throw a3;
        });
        if (200 !== e2.status) throw new g.JOSEError("Expected 200 OK from the JSON Web Key Set HTTP response");
        try {
          return await e2.json();
        } catch {
          throw new g.JOSEError("Failed to parse the JSON Web Key Set HTTP response as JSON");
        }
      }
      let aa = Symbol();
      class ab {
        ...
        #z;
        constructor(a2, b2) {
          if (!(a2 instanceof URL)) throw TypeError("url must be an instance of URL");
          this.#q = new URL(a2.href), this.#r = "number" == typeof b2?.timeoutDuration ? b2?.timeoutDuration : 5e3, this.#s = "number" == typeof b2?.cooldownDuration ? b2?.cooldownDuration : 3e4, this.#t = "number" == typeof b2?.cacheMaxAge ? b2?.cacheMaxAge : 6e5, this.#w = new Headers(b2?.headers), d && !this.#w.has("User-Agent") && this.#w.set("User-Agent", d), this.#w.has("accept") || (this.#w.set("accept", "application/json"), this.#w.append("accept", "application/jwk-set+json")), this.#x = b2?.[$], b2?.[aa] !== void 0 && (this.#z = b2?.[aa], function(a3, b3) {
            return !("object" != typeof a3 || null === a3 || !("uat" in a3) || "number" != typeof a3.uat || Date.now() - a3.uat >= b3) && "jwks" in a3 && !!(0, h.G)(a3.jwks) && !!Array.isArray(a3.jwks.keys) && !!Array.prototype.every.call(a3.jwks.keys, h.G);
          }(b2?.[aa], this.#t) && (this.#u = this.#z.uat, this.#y = Z(this.#z.jwks)));
        }
        pendingFetch() {
          return !!this.#v;
        }
        coolingDown() {
          return "number" == typeof this.#u && Date.now() < this.#u + this.#s;
        }
        fresh() {
          return "number" == typeof this.#u && Date.now() < this.#u + this.#t;
        }
        jwks() {
          return this.#y?.jwks();
        }
        async getKey(a2, b2) {
          this.#y && this.fresh() || await this.reload();
          try {
            return await this.#y(a2, b2);
          } catch (c2) {
            if (c2 instanceof g.JWKSNoMatchingKey && false === this.coolingDown()) return await this.reload(), this.#y(a2, b2);
            throw c2;
          }
        }
        async reload() {
          this.#v && ("undefined" != typeof WebSocketPair || "undefined" != typeof navigator && "Cloudflare-Workers" === navigator.userAgent) && (this.#v = void 0), this.#v ||= _(this.#q.href, this.#w, AbortSignal.timeout(this.#r), this.#x).then((a2) => {
            this.#y = Z(a2), this.#z && (this.#z.uat = Date.now(), this.#z.jwks = a2), this.#u = Date.now(), this.#v = void 0;
          }).catch((a2) => {
            throw this.#v = void 0, a2;
          }), await this.#v;
        }
      }
      function ac(a2, b2) {
        let c2 = new ab(a2, b2), d2 = async (a3, b3) => c2.getKey(a3, b3);
        return Object.defineProperties(d2, { coolingDown: { get: () => c2.coolingDown(), enumerable: true, configurable: false }, fresh: { get: () => c2.fresh(), enumerable: true, configurable: false }, reload: { value: () => c2.reload(), enumerable: true, configurable: false, writable: false }, reloading: { get: () => c2.pendingFetch(), enumerable: true, configurable: false }, jwks: { value: () => c2.jwks(), enumerable: true, configurable: false, writable: false } }), d2;
      }
      class ad {
        #n;
        constructor(a2 = {}) {
        ...
        encode() {
          let a2 = o.encode(JSON.stringify({ alg: "none" })), b2 = o.encode(this.#n.data());
          return `${a2}.${b2}.`;
        }
        setIssuer(a2) {
          return this.#n.iss = a2, this;
        ...
        static decode(a2, b2) {
          let c2;
          if ("string" != typeof a2) throw new g.JWTInvalid("Unsecured JWT must be a string");
          let { 0: d2, 1: e2, 2: f2, length: h2 } = a2.split(".");
          if (3 !== h2 || "" !== f2) throw new g.JWTInvalid("Invalid Unsecured JWT");
          try {
            if (c2 = JSON.parse(E.D0.decode(o.decode(d2))), "none" !== c2.alg) throw Error();
          } catch {
            throw new g.JWTInvalid("Invalid Unsecured JWT");
          }
          return { payload: (0, J.k6)(c2, o.decode(e2), b2), header: c2 };
        }
      }
      function ae(a2) {
        let b2;
        ...
        } else if ("object" == typeof a2 && a2) if ("protected" in a2) b2 = a2.protected;
        else throw TypeError("Token does not contain a Protected Header");
        try {
          if ("string" != typeof b2 || !b2) throw Error();
          let a3 = JSON.parse(E.D0.decode((0, o.decode)(b2)));
          if (!(0, h.G)(a3)) throw Error();
          return a3;
        } catch {
      ...
      function af(a2) {
        let b2, c2;
        if ("string" != typeof a2) throw new g.JWTInvalid("JWTs must use Compact JWS serialization, JWT must be a string");
        let { 1: d2, length: e2 } = a2.split(".");
        if (5 === e2) throw new g.JWTInvalid("Only JWTs using Compact JWS serialization can be decoded");
        if (3 !== e2) throw new g.JWTInvalid("Invalid JWT");
        if (!d2) throw new g.JWTInvalid("JWTs must contain a payload");
        try {
          b2 = (0, o.decode)(d2);
        } catch {
          throw new g.JWTInvalid("Failed to base64url decode the payload");
        }
        try {
          c2 = JSON.parse(E.D0.decode(b2));
        } catch {
          throw new g.JWTInvalid("Failed to parse the decoded payload as JSON");
        }
        if (!(0, h.G)(c2)) throw new g.JWTInvalid("Invalid JWT Claims Set");
        return c2;
      }
      function ag(a2) {
        let b2 = a2?.modulusLength ?? 2048;
        if ("number" != typeof b2 || b2 < 2048) throw new g.JOSENotSupported("Invalid or unsupported modulusLength option provided, 2048 bits or larger keys must be used");
        return b2;
      }
      async function ah(a2, b2) {
        let c2, d2;
        switch (a2) {
          case "PS256":
          case "PS384":
          case "PS512":
            c2 = { name: "RSA-PSS", hash: `SHA-${a2.slice(-3)}`, publicExponent: Uint8Array.of(1, 0, 1), modulusLength: ag(b2) }, d2 = ["sign", "verify"];
            break;
          case "RS256":
          case "RS384":
          case "RS512":
            c2 = { name: "RSASSA-PKCS1-v1_5", hash: `SHA-${a2.slice(-3)}`, publicExponent: Uint8Array.of(1, 0, 1), modulusLength: ag(b2) }, d2 = ["sign", "verify"];
            break;
          case "RSA-OAEP":
          case "RSA-OAEP-256":
          case "RSA-OAEP-384":
          case "RSA-OAEP-512":
            c2 = { name: "RSA-OAEP", hash: `SHA-${parseInt(a2.slice(-3), 10) || 1}`, publicExponent: Uint8Array.of(1, 0, 1), modulusLength: ag(b2) }, d2 = ["decrypt", "unwrapKey", "encrypt", "wrapKey"];
            break;
          case "ES256":
          ...
          case "Ed25519":
          case "EdDSA":
            d2 = ["sign", "verify"], c2 = { name: "Ed25519" };
            break;
          case "ML-DSA-44":
          case "ML-DSA-65":
          case "ML-DSA-87":
            d2 = ["sign", "verify"], c2 = { name: a2 };
            break;
          case "ECDH-ES":
          case "ECDH-ES+A128KW":
          case "ECDH-ES+A192KW":
          case "ECDH-ES+A256KW": {
            ...
            switch (a3) {
              ...
              case "P-384":
              case "P-521":
                c2 = { name: "ECDH", namedCurve: a3 };
                break;
              case "X25519":
                c2 = { name: "X25519" };
                break;
              default:
                throw new g.JOSENotSupported("Invalid or unsupported crv option provided, supported values are P-256, P-384, P-521, and X25519");
            }
            break;
          }
          default:
            throw new g.JOSENotSupported('Invalid or unsupported JWK "alg" (Algorithm) Parameter value');
        }
        return crypto.subtle.generateKey(c2, b2?.extractable ?? false, d2);
      }
      async function ai(a2, b2) {
        let c2, d2, e2;
        switch (a2) {
          case "HS256":
          case "HS384":
          case "HS512":
            c2 = parseInt(a2.slice(-3), 10), d2 = { name: "HMAC", hash: `SHA-${c2}`, length: c2 }, e2 = ["sign", "verify"];
            break;
          case "A128CBC-HS256":
          case "A192CBC-HS384":
          case "A256CBC-HS512":
            return c2 = parseInt(a2.slice(-3), 10), crypto.getRandomValues(new Uint8Array(c2 >> 3));
          case "A128KW":
          case "A192KW":
          case "A256KW":
            d2 = { name: "AES-KW", length: c2 = parseInt(a2.slice(1, 4), 10) }, e2 = ["wrapKey", "unwrapKey"];
            break;
          case "A128GCMKW":
          case "A192GCMKW":
          case "A256GCMKW":
          ...
          case "A192GCM":
          case "A256GCM":
            d2 = { name: "AES-GCM", length: c2 = parseInt(a2.slice(1, 4), 10) }, e2 = ["encrypt", "decrypt"];
            break;
          default:
            throw new g.JOSENotSupported('Invalid or unsupported JWK "alg" (Algorithm) Parameter value');
        }
        return crypto.subtle.generateKey(d2, b2?.extractable ?? false, e2);
      }
      let aj = "WebCryptoAPI";
    }, 4449: (a, b, c) => {
      var d;
      ...
      "use strict";
      c.d(b, { B: () => g });
      var d = c(6063), e = c(3169), f = c(9663);
      async function g(a2, b2, c2) {
        let g2 = await (0, d.d)(a2, b2, c2), h = (0, e.k6)(g2.protectedHeader, g2.plaintext, c2), { protectedHeader: i } = g2;
        if (void 0 !== i.iss && i.iss !== h.iss) throw new f.JWTClaimValidationFailed('replicated "iss" claim header parameter mismatch', h, "iss", "mismatch");
        if (void 0 !== i.sub && i.sub !== h.sub) throw new f.JWTClaimValidationFailed('replicated "sub" claim header parameter mismatch', h, "sub", "mismatch");
        if (void 0 !== i.aud && JSON.stringify(i.aud) !== JSON.stringify(h.aud)) throw new f.JWTClaimValidationFailed('replicated "aud" claim header parameter mismatch', h, "aud", "mismatch");
        let j = { payload: h, protectedHeader: i };
        return "function" == typeof b2 ? { ...j, key: g2.key } : j;
      }
    }, 5036: (a, b, c) => {
      "use strict";
      function d(a2) {
        if (Uint8Array.prototype.toBase64) return a2.toBase64();
        let b2 = [];
        for (let c2 = 0; c2 < a2.length; c2 += 32768) b2.push(String.fromCharCode.apply(null, a2.subarray(c2, c2 + 32768)));
        return btoa(b2.join(""));
      }
      function e(a2) {
        if (Uint8Array.fromBase64) return Uint8Array.fromBase64(a2);
      ...
      class f {
        ...
        async encrypt(a2, b2) {
          let c2 = new d.j(this.#n.data());
          return this.#c && (this.#A || this.#B || this.#C) && (this.#c = { ...this.#c, iss: this.#A ? this.#n.iss : void 0, sub: this.#B ? this.#n.sub : void 0, aud: this.#C ? this.#n.aud : void 0 }), c2.setProtectedHeader(this.#c), this.#h && c2.setInitializationVector(this.#h), this.#g && c2.setContentEncryptionKey(this.#g), this.#i && c2.setKeyManagementParameters(this.#i), c2.encrypt(a2, b2);
        }
      }
    }, 5356: (a) => {
      "use strict";
      ...
      "use strict";
      c.d(b, { Y: () => n });
      var d = c(8629), e = c(1096), f = c(2356), g = c(9663), h = c(9814), i = c(269), j = c(6495);
      async function k(a2, b2) {
        if (!(a2 instanceof Uint8Array)) throw TypeError("First argument must be a buffer");
        if (!(b2 instanceof Uint8Array)) throw TypeError("Second argument must be a buffer");
        let c2 = { name: "HMAC", hash: "SHA-256" }, d2 = await crypto.subtle.generateKey(c2, false, ["sign"]), e2 = new Uint8Array(await crypto.subtle.sign(c2, d2, a2)), f2 = new Uint8Array(await crypto.subtle.sign(c2, d2, b2)), g2 = 0, h2 = -1;
        for (; ++h2 < 32; ) g2 |= e2[h2] ^ f2[h2];
        return 0 === g2;
      }
      async function l(a2, b2, c2, e2, f2, h2) {
        let j2, l2;
        if (!(b2 instanceof Uint8Array)) throw TypeError((0, i.X)(b2, "Uint8Array"));
        let m2 = parseInt(a2.slice(1, 4), 10), n2 = await crypto.subtle.importKey("raw", b2.subarray(m2 >> 3), "AES-CBC", false, ["decrypt"]), o = await crypto.subtle.importKey("raw", b2.subarray(0, m2 >> 3), { hash: `SHA-${m2 << 1}`, name: "HMAC" }, false, ["sign"]), p = (0, d.xW)(h2, e2, c2, (0, d.mx)(h2.length << 3)), q = new Uint8Array((await crypto.subtle.sign("HMAC", o, p)).slice(0, m2 >> 3));
        try {
          j2 = await k(f2, q);
        } catch {
        }
        if (!j2) throw new g.JWEDecryptionFailed();
        try {
          l2 = new Uint8Array(await crypto.subtle.decrypt({ iv: e2, name: "AES-CBC" }, n2, c2));
        } catch {
        }
        if (!l2) throw new g.JWEDecryptionFailed();
        return l2;
      }
      async function m(a2, b2, c2, e2, f2, i2) {
        let j2;
        b2 instanceof Uint8Array ? j2 = await crypto.subtle.importKey("raw", b2, "AES-GCM", false, ["decrypt"]) : ((0, h.$)(b2, a2, "decrypt"), j2 = b2);
        try {
          return new Uint8Array(await crypto.subtle.decrypt({ additionalData: i2, iv: e2, name: "AES-GCM", tagLength: 128 }, j2, (0, d.xW)(c2, f2)));
        } catch {
          throw new g.JWEDecryptionFailed();
        }
      }
      async function n(a2, b2, c2, d2, h2, k2) {
        if (!(0, j.R7)(b2) && !(b2 instanceof Uint8Array)) throw TypeError((0, i.X)(b2, "CryptoKey", "KeyObject", "Uint8Array", "JSON Web Key"));
        if (!d2) throw new g.JWEInvalid("JWE Initialization Vector missing");
        if (!h2) throw new g.JWEInvalid("JWE Authentication Tag missing");
        switch ((0, e.S)(a2, d2), a2) {
          case "A128CBC-HS256":
          case "A192CBC-HS384":
          case "A256CBC-HS512":
            return b2 instanceof Uint8Array && (0, f.W)(b2, parseInt(a2.slice(-3), 10)), l(a2, b2, c2, d2, h2, k2);
          case "A128GCM":
          case "A192GCM":
          case "A256GCM":
            return b2 instanceof Uint8Array && (0, f.W)(b2, parseInt(a2.slice(1, 4), 10)), m(a2, b2, c2, d2, h2, k2);
          default:
            throw new g.JOSENotSupported("Unsupported JWE Content Encryption Algorithm");
        }
      }
    }, 5461: (a, b, c) => {
      "use strict";
      async function d(a2, b2) {
        let c2 = `SHA-${a2.slice(-3)}`;
        return new Uint8Array(await crypto.subtle.digest(c2, b2));
      }
      c.d(b, { b: () => d });
    }, 5475: (a, b, c) => {
      "use strict";
      c.d(b, { L: () => j, o: () => k });
      var d = c(9148), e = c(9986), f = c(9814), g = c(8629), h = c(9663);
      async function i(a2, b2, c2, d2) {
        if (!(a2 instanceof Uint8Array) || a2.length < 8) throw new h.JWEInvalid("PBES2 Salt Input must be 8 or more octets");
        let e2 = (0, g.xW)((0, g.lF)(b2), Uint8Array.of(0), a2), i2 = parseInt(b2.slice(13, 16), 10), j2 = { hash: `SHA-${b2.slice(8, 11)}`, iterations: c2, name: "PBKDF2", salt: e2 }, k2 = await (d2 instanceof Uint8Array ? crypto.subtle.importKey("raw", d2, "PBKDF2", false, ["deriveBits"]) : ((0, f.$)(d2, b2, "deriveBits"), d2));
        return new Uint8Array(await crypto.subtle.deriveBits(j2, k2, i2));
      }
      async function j(a2, b2, c2, f2 = 2048, g2 = crypto.getRandomValues(new Uint8Array(16))) {
        let h2 = await i(g2, a2, f2, b2);
        return { encryptedKey: await e.L(a2.slice(-6), h2, c2), p2c: f2, p2s: (0, d.encode)(g2) };
      }
      async function k(a2, b2, c2, d2, f2) {
        let g2 = await i(f2, a2, d2, b2);
        return e.o(a2.slice(-6), g2, c2);
      }
    }, 5521: (a) => {
      "use strict";
      ...
      "use strict";
      c.d(b, { d: () => g });
      var d = c(9578), e = c(9663), f = c(8629);
      async function g(a2, b2, c2) {
        if (a2 instanceof Uint8Array && (a2 = f.D0.decode(a2)), "string" != typeof a2) throw new e.JWEInvalid("Compact JWE must be a string or Uint8Array");
        let { 0: g2, 1: h, 2: i, 3: j, 4: k, length: l } = a2.split(".");
        if (5 !== l) throw new e.JWEInvalid("Invalid Compact JWE");
        let m = await (0, d.d)({ ciphertext: j, iv: i || void 0, protected: g2, tag: k || void 0, encrypted_key: h || void 0 }, b2, c2), n = { plaintext: m.plaintext, protectedHeader: m.protectedHeader };
        return "function" == typeof b2 ? { ...n, key: m.key } : n;
      }
    }, 6121: (a, b, c) => {
      "use strict";
      c.d(b, { H: () => i, T: () => j });
      var d = c(8629), e = c(9814), f = c(5461);
      function g(a2) {
        return (0, d.xW)((0, d.VS)(a2.length), a2);
      }
      async function h(a2, b2, c2) {
        let e2 = b2 >> 3, g2 = Math.ceil(e2 / 32), h2 = new Uint8Array(32 * g2);
        for (let b3 = 1; b3 <= g2; b3++) {
          let e3 = new Uint8Array(4 + a2.length + c2.length);
          e3.set((0, d.VS)(b3), 0), e3.set(a2, 4), e3.set(c2, 4 + a2.length);
          let g3 = await (0, f.b)("sha256", e3);
          h2.set(g3, (b3 - 1) * 32);
        }
        return h2.slice(0, e2);
      }
      async function i(a2, b2, c2, f2, j2 = new Uint8Array(), k = new Uint8Array()) {
        var l;
        (0, e.$)(a2, "ECDH"), (0, e.$)(b2, "ECDH", "deriveBits");
        let m = g((0, d.lF)(c2)), n = g(j2), o = g(k), p = (0, d.VS)(f2), q = new Uint8Array(), r = (0, d.xW)(m, n, o, p, q);
        return h(new Uint8Array(await crypto.subtle.deriveBits({ name: a2.algorithm.name, public: a2 }, b2, "X25519" === (l = a2).algorithm.name ? 256 : Math.ceil(parseInt(l.algorithm.namedCurve.slice(-3), 10) / 8) << 3)), f2, r);
      }
      function j(a2) {
        switch (a2.algorithm.namedCurve) {
          case "P-256":
          case "P-384":
          case "P-521":
            return true;
          default:
            return "X25519" === a2.algorithm.name;
        }
      }
    }, 6144: (a, b, c) => {
      "use strict";
      let d;
      c.d(b, { l: () => j });
      var e = c(3652), f = c(9148), g = c(2877), h = c(6495);
      let i = async (a2, b2, c2, e2 = false) => {
        let f2 = (d ||= /* @__PURE__ */ new WeakMap()).get(a2);
        if (f2?.[c2]) return f2[c2];
        let h2 = await (0, g.x)({ ...b2, alg: c2 });
        return e2 && Object.freeze(a2), f2 ? f2[c2] = h2 : d.set(a2, { [c2]: h2 }), h2;
      };
      async function j(a2, b2) {
        if (a2 instanceof Uint8Array || (0, h.R7)(a2)) return a2;
        if ((0, h.D2)(a2)) {
          if ("secret" === a2.type) return a2.export();
          if ("toCryptoKey" in a2 && "function" == typeof a2.toCryptoKey) try {
            return ((a3, b3) => {
              let c3, e2 = (d ||= /* @__PURE__ */ new WeakMap()).get(a3);
              if (e2?.[b3]) return e2[b3];
              let f2 = "public" === a3.type, g2 = !!f2;
              if ("x25519" === a3.asymmetricKeyType) {
                switch (b3) {
                  case "ECDH-ES":
                  case "ECDH-ES+A128KW":
                  case "ECDH-ES+A192KW":
                ...
                c3 = a3.toCryptoKey(a3.asymmetricKeyType, g2, f2 ? [] : ["deriveBits"]);
              }
              if ("ed25519" === a3.asymmetricKeyType) {
                if ("EdDSA" !== b3 && "Ed25519" !== b3) throw TypeError("given KeyObject instance cannot be used for this algorithm");
                c3 = a3.toCryptoKey(a3.asymmetricKeyType, g2, [f2 ? "verify" : "sign"]);
              }
              switch (a3.asymmetricKeyType) {
                case "ml-dsa-44":
                case "ml-dsa-65":
                case "ml-dsa-87":
                  if (b3 !== a3.asymmetricKeyType.toUpperCase()) throw TypeError("given KeyObject instance cannot be used for this algorithm");
                  c3 = a3.toCryptoKey(a3.asymmetricKeyType, g2, [f2 ? "verify" : "sign"]);
              }
              if ("rsa" === a3.asymmetricKeyType) {
                let d2;
                switch (b3) {
                  case "RSA-OAEP":
                    d2 = "SHA-1";
                ...
                if (b3.startsWith("RSA-OAEP")) return a3.toCryptoKey({ name: "RSA-OAEP", hash: d2 }, g2, f2 ? ["encrypt"] : ["decrypt"]);
                c3 = a3.toCryptoKey({ name: b3.startsWith("PS") ? "RSA-PSS" : "RSASSA-PKCS1-v1_5", hash: d2 }, g2, [f2 ? "verify" : "sign"]);
              }
              if ("ec" === a3.asymmetricKeyType) {
                let d2 = (/* @__PURE__ */ new Map([["prime256v1", "P-256"], ["secp384r1", "P-384"], ["secp521r1", "P-521"]])).get(a3.asymmetricKeyDetails?.namedCurve);
                if (!d2) throw TypeError("given KeyObject instance cannot be used for this algorithm");
                "ES256" === b3 && "P-256" === d2 && (c3 = a3.toCryptoKey({ name: "ECDSA", namedCurve: d2 }, g2, [f2 ? "verify" : "sign"])), "ES384" === b3 && "P-384" === d2 && (c3 = a3.toCryptoKey({ name: "ECDSA", namedCurve: d2 }, g2, [f2 ? "verify" : "sign"])), "ES512" === b3 && "P-521" === d2 && (c3 = a3.toCryptoKey({ name: "ECDSA", namedCurve: d2 }, g2, [f2 ? "verify" : "sign"])), b3.startsWith("ECDH-ES") && (c3 = a3.toCryptoKey({ name: "ECDH", namedCurve: d2 }, g2, f2 ? [] : ["deriveBits"]));
              }
              if (!c3) throw TypeError("given KeyObject instance cannot be used for this algorithm");
              return e2 ? e2[b3] = c3 : d.set(a3, { [b3]: c3 }), c3;
            })(a2, b2);
          } catch (a3) {
            if (a3 instanceof TypeError) throw a3;
          }
          let c2 = a2.export({ format: "jwk" });
          return i(a2, c2, b2);
        }
        if ((0, e.ll)(a2)) return a2.k ? (0, f.decode)(a2.k) : i(a2, a2, b2, true);
        throw Error("unreachable");
      }
    }, 6218: (a, b, c) => {
      "use strict";
      c.d(b, { Z: () => q });
      var d = c(9148), e = c(8767), f = c(2934), g = c(6805);
      async function h(a2, b2, c2, d2) {
        let h2 = await (0, g.j)(a2, b2, "verify");
        (0, f.O)(a2, h2);
        let i2 = (0, e.B)(a2, h2.algorithm);
        try {
          return await crypto.subtle.verify(i2, h2, c2, d2);
        } catch {
          return false;
        }
      }
      var i = c(9663), j = c(8629), k = c(798), l = c(1967), m = c(3910), n = c(2072), o = c(4070), p = c(6144);
      async function q(a2, b2, c2) {
        let e2, f2;
        if (!(0, l.G)(a2)) throw new i.JWSInvalid("Flattened JWS must be an object");
        if (void 0 === a2.protected && void 0 === a2.header) throw new i.JWSInvalid('Flattened JWS must have either of the "protected" or "header" members');
        if (void 0 !== a2.protected && "string" != typeof a2.protected) throw new i.JWSInvalid("JWS Protected Header incorrect type");
        if (void 0 === a2.payload) throw new i.JWSInvalid("JWS Payload missing");
        if ("string" != typeof a2.signature) throw new i.JWSInvalid("JWS Signature missing or incorrect type");
        if (void 0 !== a2.header && !(0, l.G)(a2.header)) throw new i.JWSInvalid("JWS Unprotected Header incorrect type");
        let g2 = {};
        if (a2.protected) try {
          let b3 = (0, d.decode)(a2.protected);
          g2 = JSON.parse(j.D0.decode(b3));
        } catch {
          throw new i.JWSInvalid("JWS Protected Header is invalid");
        }
        if (!(0, k.f)(g2, a2.header)) throw new i.JWSInvalid("JWS Protected and JWS Unprotected Header Parameter names must be disjoint");
        let q2 = { ...g2, ...a2.header }, r = (0, n.n)(i.JWSInvalid, /* @__PURE__ */ new Map([["b64", true]]), c2?.crit, g2, q2), s = true;
        if (r.has("b64") && "boolean" != typeof (s = g2.b64)) throw new i.JWSInvalid('The "b64" (base64url-encode payload) Header Parameter must be a boolean');
        let { alg: t } = q2;
        if ("string" != typeof t || !t) throw new i.JWSInvalid('JWS "alg" (Algorithm) Header Parameter missing or invalid');
        let u = c2 && (0, o.F)("algorithms", c2.algorithms);
        if (u && !u.has(t)) throw new i.JOSEAlgNotAllowed('"alg" (Algorithm) Header Parameter value not allowed');
        if (s) {
          if ("string" != typeof a2.payload) throw new i.JWSInvalid("JWS Payload must be a string");
        } else if ("string" != typeof a2.payload && !(a2.payload instanceof Uint8Array)) throw new i.JWSInvalid("JWS Payload must be a string or an Uint8Array instance");
        let v = false;
        "function" == typeof b2 && (b2 = await b2(g2, a2), v = true), (0, m.y)(t, b2, "verify");
        let w = (0, j.xW)(void 0 !== a2.protected ? (0, j.lF)(a2.protected) : new Uint8Array(), (0, j.lF)("."), "string" == typeof a2.payload ? s ? (0, j.lF)(a2.payload) : j.Rd.encode(a2.payload) : a2.payload);
        try {
          e2 = (0, d.decode)(a2.signature);
        } catch {
          throw new i.JWSInvalid("Failed to base64url decode the signature");
        }
        let x = await (0, p.l)(b2, t);
        if (!await h(t, x, e2, w)) throw new i.JWSSignatureVerificationFailed();
        if (s) try {
          f2 = (0, d.decode)(a2.payload);
        } catch {
          throw new i.JWSInvalid("Failed to base64url decode the payload");
        }
        else f2 = "string" == typeof a2.payload ? j.Rd.encode(a2.payload) : a2.payload;
        let y = { payload: f2 };
        return (void 0 !== a2.protected && (y.protectedHeader = g2), void 0 !== a2.header && (y.unprotectedHeader = a2.header), v) ? { ...y, key: x } : y;
      }
    }, 6391: (a, b, c) => {
      "use strict";
      c.d(b, { L: () => g, o: () => h });
      var d = c(2894), e = c(5414), f = c(9148);
      async function g(a2, b2, c2, e2) {
        let g2 = a2.slice(0, 7), h2 = await (0, d.w)(g2, c2, b2, e2, new Uint8Array());
        return { encryptedKey: h2.ciphertext, iv: (0, f.encode)(h2.iv), tag: (0, f.encode)(h2.tag) };
      }
      async function h(a2, b2, c2, d2, f2) {
        let g2 = a2.slice(0, 7);
        return (0, e.Y)(g2, b2, c2, d2, f2, new Uint8Array());
      }
    }, 6440: (a, b) => {
      "use strict";
      ...
      "use strict";
      function d(a2) {
        if (!e(a2)) throw Error("CryptoKey instance expected");
      }
      c.d(b, { D2: () => f, P2: () => g, R7: () => e, Tz: () => d });
      let e = (a2) => {
        if (a2?.[Symbol.toStringTag] === "CryptoKey") return true;
        try {
          return a2 instanceof CryptoKey;
        } catch {
          return false;
        }
      }, f = (a2) => a2?.[Symbol.toStringTag] === "KeyObject", g = (a2) => e(a2) || f(a2);
    }, 6805: (a, b, c) => {
      "use strict";
      c.d(b, { j: () => f });
      var d = c(9814), e = c(269);
      async function f(a2, b2, c2) {
        if (b2 instanceof Uint8Array) {
          if (!a2.startsWith("HS")) throw TypeError((0, e.X)(b2, "CryptoKey", "KeyObject", "JSON Web Key"));
          return crypto.subtle.importKey("raw", b2, { hash: `SHA-${a2.slice(-3)}`, name: "HMAC" }, false, [c2]);
        }
        return (0, d.Y)(b2, a2, c2), b2;
      }
    }, 7276: (a, b, c) => {
      "use strict";
      c.d(b, { V: () => g });
      var d = c(9035), e = c(3169), f = c(9663);
      async function g(a2, b2, c2) {
        let g2 = await (0, d.B)(a2, b2, c2);
        if (g2.protectedHeader.crit?.includes("b64") && false === g2.protectedHeader.b64) throw new f.JWTInvalid("JWTs MUST NOT use unencoded payload");
        let h = { payload: (0, e.k6)(g2.protectedHeader, g2.payload, c2), protectedHeader: g2.protectedHeader };
        return "function" == typeof b2 ? { ...h, key: g2.key } : h;
      }
    }, 7463: (a, b, c) => {
      "use strict";
      c.d(b, { j: () => e });
      ...
      class e {
        ...
        setContentEncryptionKey(a2) {
          return this.#l.setContentEncryptionKey(a2), this;
        }
        setInitializationVector(a2) {
          return this.#l.setInitializationVector(a2), this;
        }
        setProtectedHeader(a2) {
          return this.#l.setProtectedHeader(a2), this;
        }
        setKeyManagementParameters(a2) {
          return this.#l.setKeyManagementParameters(a2), this;
        }
        async encrypt(a2, b2) {
          let c2 = await this.#l.encrypt(a2, b2);
      ...
      "use strict";
      c.d(b, { D0: () => e, Rd: () => d, VS: () => i, lF: () => j, mx: () => h, xW: () => f });
      let d = new TextEncoder(), e = new TextDecoder();
      function f(...a2) {
        let b2 = new Uint8Array(a2.reduce((a3, { length: b3 }) => a3 + b3, 0)), c2 = 0;
        for (let d2 of a2) b2.set(d2, c2), c2 += d2.length;
        return b2;
      }
      function g(a2, b2, c2) {
        if (b2 < 0 || b2 >= 4294967296) throw RangeError(`value must be >= 0 and <= ${4294967296 - 1}. Received ${b2}`);
        a2.set([b2 >>> 24, b2 >>> 16, b2 >>> 8, 255 & b2], c2);
      }
      function h(a2) {
        let b2 = Math.floor(a2 / 4294967296), c2 = new Uint8Array(8);
        return g(c2, b2, 0), g(c2, a2 % 4294967296, 4), c2;
      }
      function i(a2) {
      ...
      function j(a2) {
        let b2 = new Uint8Array(a2.length);
        for (let c2 = 0; c2 < a2.length; c2++) {
          let d2 = a2.charCodeAt(c2);
      ...
      function e(a2, b2) {
        ...
        switch (a2) {
          ...
          case "PS384":
          case "PS512":
            return { hash: c2, name: "RSA-PSS", saltLength: parseInt(a2.slice(-3), 10) >> 3 };
          case "RS256":
          case "RS384":
          case "RS512":
          ...
          case "ML-DSA-87":
            return { name: a2 };
          default:
            throw new d.JOSENotSupported(`alg ${a2} is not supported either by JOSE or your javascript runtime`);
        }
      }
    }, 8774: (a, b, c) => {
      "use strict";
      c.d(b, { IG: () => u, Jq: () => k, Nj: () => j, sc: () => v, zp: () => w });
      var d = c(269), e = c(5036), f = c(9663), g = c(6495);
      let h = (a2, b2) => {
        let c2 = (a2.match(/.{1,64}/g) || []).join("\n");
        return `-----BEGIN ${b2}-----
${c2}
-----END ${b2}-----`;
      }, i = async (a2, b2, c2) => {
        if ((0, g.D2)(c2)) {
          if (c2.type !== a2) throw TypeError(`key is not a ${a2} key`);
          return c2.export({ format: "pem", type: b2 });
        }
        if (!(0, g.R7)(c2)) throw TypeError((0, d.X)(c2, "CryptoKey", "KeyObject"));
        if (!c2.extractable) throw TypeError("CryptoKey is not extractable");
        if (c2.type !== a2) throw TypeError(`key is not a ${a2} key`);
        return h((0, e.W)(new Uint8Array(await crypto.subtle.exportKey(b2, c2))), `${a2.toUpperCase()} KEY`);
      }, j = (a2) => i("public", "spki", a2), k = (a2) => i("private", "pkcs8", a2), l = (a2, b2) => {
        if (a2.byteLength !== b2.length) return false;
        for (let c2 = 0; c2 < a2.byteLength; c2++) if (a2[c2] !== b2[c2]) return false;
        return true;
      }, m = (a2) => ({ data: a2, pos: 0 }), n = (a2) => {
        let b2 = a2.data[a2.pos++];
        if (128 & b2) {
          let c2 = 127 & b2, d2 = 0;
          for (let b3 = 0; b3 < c2; b3++) d2 = d2 << 8 | a2.data[a2.pos++];
          return d2;
        }
        return b2;
      }, o = (a2, b2 = 1) => {
        ...
        a2.pos++;
        let c2 = n(a2);
        a2.pos += c2, b2 > 1 && o(a2, b2 - 1);
      }, p = (a2, b2, c2) => {
        if (a2.data[a2.pos++] !== b2) throw Error(c2);
      }, q = (a2, b2) => {
        let c2 = a2.data.subarray(a2.pos, a2.pos + b2);
        return a2.pos += b2, c2;
      }, r = (a2) => {
        let b2 = ((a3) => {
          p(a3, 6, "Expected algorithm OID");
          let b3 = n(a3);
          return q(a3, b3);
        })(a2);
        if (l(b2, [43, 101, 110])) return "X25519";
        if (!l(b2, [42, 134, 72, 206, 61, 2, 1])) throw Error("Unsupported key algorithm");
        p(a2, 6, "Expected curve OID");
        let c2 = n(a2), d2 = q(a2, c2);
        for (let { name: a3, oid: b3 } of [{ name: "P-256", oid: [42, 134, 72, 206, 61, 3, 1, 7] }, { name: "P-384", oid: [43, 129, 4, 0, 34] }, { name: "P-521", oid: [43, 129, 4, 0, 35] }]) if (l(d2, b3)) return a3;
        throw Error("Unsupported named curve");
      }, s = async (a2, b2, c2, d2) => {
        let e2, g2, h2 = "spki" === a2, i2 = () => h2 ? ["verify"] : ["sign"];
        switch (c2) {
          case "PS256":
          case "PS384":
          case "PS512":
            e2 = { name: "RSA-PSS", hash: `SHA-${c2.slice(-3)}` }, g2 = i2();
            break;
          case "RS256":
          case "RS384":
          case "RS512":
            e2 = { name: "RSASSA-PKCS1-v1_5", hash: `SHA-${c2.slice(-3)}` }, g2 = i2();
            break;
          case "RSA-OAEP":
          case "RSA-OAEP-256":
          case "RSA-OAEP-384":
          case "RSA-OAEP-512":
            e2 = { name: "RSA-OAEP", hash: `SHA-${parseInt(c2.slice(-3), 10) || 1}` }, g2 = h2 ? ["encrypt", "wrapKey"] : ["decrypt", "unwrapKey"];
            break;
          case "ES256":
          case "ES384":
          case "ES512":
            e2 = { name: "ECDSA", namedCurve: { ES256: "P-256", ES384: "P-384", ES512: "P-521" }[c2] }, g2 = i2();
            break;
          case "ECDH-ES":
          case "ECDH-ES+A128KW":
          case "ECDH-ES+A192KW":
          case "ECDH-ES+A256KW":
            try {
              let a3 = d2.getNamedCurve(b2);
              e2 = "X25519" === a3 ? { name: "X25519" } : { name: "ECDH", namedCurve: a3 };
            } catch (a3) {
              throw new f.JOSENotSupported("Invalid or unsupported key format");
            }
            g2 = h2 ? [] : ["deriveBits"];
            break;
          case "Ed25519":
          case "EdDSA":
            e2 = { name: "Ed25519" }, g2 = i2();
            break;
          case "ML-DSA-44":
          case "ML-DSA-65":
          case "ML-DSA-87":
            ...
            break;
          default:
            throw new f.JOSENotSupported('Invalid or unsupported "alg" (Algorithm) value');
        }
        return crypto.subtle.importKey(a2, b2, e2, d2?.extractable ?? !!h2, g2);
      }, t = (a2, b2) => (0, e.y)(a2.replace(b2, "")), u = (a2, b2, c2) => {
        let d2 = t(a2, /(?:-----(?:BEGIN|END) PRIVATE KEY-----|\s)/g), e2 = c2;
        return b2?.startsWith?.("ECDH-ES") && ((e2 ||= {}).getNamedCurve = (a3) => {
          let b3 = m(a3);
          return !function(a4) {
            p(a4, 48, "Invalid PKCS#8 structure"), n(a4), p(a4, 2, "Expected version field");
            let b4 = n(a4);
            a4.pos += b4, p(a4, 48, "Expected algorithm identifier"), n(a4), a4.pos;
          }(b3), r(b3);
        }), s("pkcs8", d2, b2, e2);
      }, v = (a2, b2, c2) => {
        let d2 = t(a2, /(?:-----(?:BEGIN|END) PUBLIC KEY-----|\s)/g), e2 = c2;
        return b2?.startsWith?.("ECDH-ES") && ((e2 ||= {}).getNamedCurve = (a3) => {
          let b3 = m(a3);
          return !function(a4) {
            p(a4, 48, "Invalid SPKI structure"), n(a4), p(a4, 48, "Expected algorithm identifier"), n(a4), a4.pos;
          }(b3), r(b3);
        }), s("spki", d2, b2, e2);
      }, w = (a2, b2, c2) => {
        let d2;
        try {
          d2 = function(a3) {
            var b3 = t(a3, /(?:-----(?:BEGIN|END) CERTIFICATE-----|\s)/g);
            let c3 = m(b3);
            p(c3, 48, "Invalid certificate structure"), n(c3), p(c3, 48, "Invalid tbsCertificate structure"), n(c3), 160 === b3[c3.pos] ? o(c3, 6) : o(c3, 5);
            let d3 = c3.pos;
            p(c3, 48, "Invalid SPKI structure");
            let e2 = n(c3);
            return b3.subarray(d3, d3 + e2 + (c3.pos - d3));
          }(a2);
        } catch (a3) {
          throw TypeError("Failed to parse the X.509 certificate", { cause: a3 });
        }
        return v(h((0, e.W)(d2), "PUBLIC KEY"), b2, c2);
      };
    }, 9035: (a, b, c) => {
      "use strict";
      c.d(b, { B: () => g });
      var d = c(6218), e = c(9663), f = c(8629);
      async function g(a2, b2, c2) {
        if (a2 instanceof Uint8Array && (a2 = f.D0.decode(a2)), "string" != typeof a2) throw new e.JWSInvalid("Compact JWS must be a string or Uint8Array");
        let { 0: g2, 1: h, 2: i, length: j } = a2.split(".");
        if (3 !== j) throw new e.JWSInvalid("Invalid Compact JWS");
        let k = await (0, d.Z)({ payload: h, protected: g2, signature: i }, b2, c2), l = { payload: k.payload, protectedHeader: k.protectedHeader };
        return "function" == typeof b2 ? { ...l, key: k.key } : l;
      }
    }, 9074: (a, b, c) => {
      "use strict";
      c.d(b, { D: () => f, u: () => e });
      ...
      "use strict";
      c.d(b, { s: () => o });
      var d = c(9986), e = c(6121), f = c(5475), g = c(703), h = c(9148), i = c(6144), j = c(9074), k = c(9663), l = c(3340), m = c(6391), n = c(6495);
      async function o(a2, b2, c2, p, q = {}) {
        let r, s, t;
        switch (a2) {
          case "dir":
            t = c2;
          ...
          case "ECDH-ES+A256KW": {
            let f2;
            if ((0, n.Tz)(c2), !e.T(c2)) throw new k.JOSENotSupported("ECDH with the provided key is not allowed or not supported by your javascript runtime");
            let { apu: g2, apv: m2 } = q;
            f2 = q.epk ? await (0, i.l)(q.epk, a2) : (await crypto.subtle.generateKey(c2.algorithm, true, ["deriveBits"])).privateKey;
            let { x: o2, y: u, crv: v, kty: w } = await (0, l.JU)(f2), x = await e.H(c2, f2, "ECDH-ES" === a2 ? b2 : a2, "ECDH-ES" === a2 ? (0, j.u)(b2) : parseInt(a2.slice(-5, -2), 10), g2, m2);
            if (s = { epk: { x: o2, crv: v, kty: w } }, "EC" === w && (s.epk.y = u), g2 && (s.apu = (0, h.encode)(g2)), m2 && (s.apv = (0, h.encode)(m2)), "ECDH-ES" === a2) {
              t = x;
              break;
            }
            t = p || (0, j.D)(b2);
          ...
          case "RSA-OAEP-384":
          case "RSA-OAEP-512":
            t = p || (0, j.D)(b2), (0, n.Tz)(c2), r = await g.w(a2, c2, t);
            break;
          ...
          case "PBES2-HS512+A256KW": {
            t = p || (0, j.D)(b2);
            let { p2c: d2, p2s: e2 } = q;
            ({ encryptedKey: r, ...s } = await f.L(a2, c2, t, d2, e2));
            break;
          }
          case "A128KW":
          case "A192KW":
          case "A256KW":
            t = p || (0, j.D)(b2), r = await d.L(a2, c2, t);
            break;
          case "A128GCMKW":
          case "A192GCMKW":
          case "A256GCMKW": {
            t = p || (0, j.D)(b2);
            let { iv: d2 } = q;
            ({ encryptedKey: r, ...s } = await (0, m.L)(a2, c2, t, d2));
            break;
          }
          default:
            throw new k.JOSENotSupported('Invalid or unsupported "alg" (JWE Algorithm) header value');
        }
        return { cek: t, encryptedKey: r, parameters: s };
      }
    }, 9148: (a, b, c) => {
      "use strict";
      c.r(b), c.d(b, { decode: () => f, encode: () => g });
      var d = c(8629), e = c(5036);
      function f(a2) {
        if (Uint8Array.fromBase64) return Uint8Array.fromBase64("string" == typeof a2 ? a2 : d.D0.decode(a2), { alphabet: "base64url" });
        let b2 = a2;
        b2 instanceof Uint8Array && (b2 = d.D0.decode(b2)), b2 = b2.replace(/-/g, "+").replace(/_/g, "/");
        try {
          return (0, e.y)(b2);
        } catch {
      ...
      function g(a2) {
        let b2 = a2;
        return ("string" == typeof b2 && (b2 = d.Rd.encode(b2)), Uint8Array.prototype.toBase64) ? b2.toBase64({ alphabet: "base64url", omitPadding: true }) : (0, e.W)(b2).replace(/=/g, "").replace(/\+/g, "-").replace(/\//g, "_");
      }
    }, 9165: (a, b, c) => {
      "use strict";
      var d = c(5356).Buffer;
      ...
      "use strict";
      c.d(b, { d: () => w });
      var d = c(9148), e = c(5414), f = c(9663), g = c(798), h = c(1967), i = c(9986), j = c(6121), k = c(5475), l = c(703), m = c(9074), n = c(194), o = c(6391), p = c(6495);
      async function q(a2, b2, c2, e2, g2) {
        switch (a2) {
          case "dir":
            if (void 0 !== c2) throw new f.JWEInvalid("Encountered unexpected JWE Encrypted Key");
            return b2;
          case "ECDH-ES":
            if (void 0 !== c2) throw new f.JWEInvalid("Encountered unexpected JWE Encrypted Key");
          case "ECDH-ES+A128KW":
          ...
          case "ECDH-ES+A256KW": {
            let g3, k2;
            if (!(0, h.G)(e2.epk)) throw new f.JWEInvalid('JOSE Header "epk" (Ephemeral Public Key) missing or invalid');
            if ((0, p.Tz)(b2), !j.T(b2)) throw new f.JOSENotSupported("ECDH with the provided key is not allowed or not supported by your javascript runtime");
            let l2 = await (0, n.Og)(e2.epk, a2);
            if ((0, p.Tz)(l2), void 0 !== e2.apu) {
              if ("string" != typeof e2.apu) throw new f.JWEInvalid('JOSE Header "apu" (Agreement PartyUInfo) invalid');
              try {
                g3 = (0, d.decode)(e2.apu);
              } catch {
                throw new f.JWEInvalid("Failed to base64url decode the apu");
              }
            }
            if (void 0 !== e2.apv) {
              if ("string" != typeof e2.apv) throw new f.JWEInvalid('JOSE Header "apv" (Agreement PartyVInfo) invalid');
              try {
                k2 = (0, d.decode)(e2.apv);
              } catch {
                throw new f.JWEInvalid("Failed to base64url decode the apv");
              }
            }
            let o2 = await j.H(l2, b2, "ECDH-ES" === a2 ? e2.enc : a2, "ECDH-ES" === a2 ? (0, m.u)(e2.enc) : parseInt(a2.slice(-5, -2), 10), g3, k2);
            if ("ECDH-ES" === a2) return o2;
            if (void 0 === c2) throw new f.JWEInvalid("JWE Encrypted Key missing");
            return i.o(a2.slice(-6), o2, c2);
          }
          case "RSA-OAEP":
          case "RSA-OAEP-256":
          case "RSA-OAEP-384":
          case "RSA-OAEP-512":
            if (void 0 === c2) throw new f.JWEInvalid("JWE Encrypted Key missing");
            return (0, p.Tz)(b2), l.Y(a2, b2, c2);
          case "PBES2-HS256+A128KW":
          case "PBES2-HS384+A192KW":
          case "PBES2-HS512+A256KW": {
            let h2;
            if (void 0 === c2) throw new f.JWEInvalid("JWE Encrypted Key missing");
            if ("number" != typeof e2.p2c) throw new f.JWEInvalid('JOSE Header "p2c" (PBES2 Count) missing or invalid');
            let i2 = g2?.maxPBES2Count || 1e4;
            if (e2.p2c > i2) throw new f.JWEInvalid('JOSE Header "p2c" (PBES2 Count) out is of acceptable bounds');
            if ("string" != typeof e2.p2s) throw new f.JWEInvalid('JOSE Header "p2s" (PBES2 Salt) missing or invalid');
            try {
              h2 = (0, d.decode)(e2.p2s);
            } catch {
              throw new f.JWEInvalid("Failed to base64url decode the p2s");
            }
            return k.o(a2, b2, c2, e2.p2c, h2);
          }
          case "A128KW":
          case "A192KW":
          case "A256KW":
          ...
          case "A256GCMKW": {
            let g3, h2;
            if (void 0 === c2) throw new f.JWEInvalid("JWE Encrypted Key missing");
            if ("string" != typeof e2.iv) throw new f.JWEInvalid('JOSE Header "iv" (Initialization Vector) missing or invalid');
            if ("string" != typeof e2.tag) throw new f.JWEInvalid('JOSE Header "tag" (Authentication Tag) missing or invalid');
            try {
              g3 = (0, d.decode)(e2.iv);
            } catch {
              throw new f.JWEInvalid("Failed to base64url decode the iv");
            }
            try {
              h2 = (0, d.decode)(e2.tag);
            } catch {
              throw new f.JWEInvalid("Failed to base64url decode the tag");
            }
            return (0, o.o)(a2, b2, c2, g3, h2);
          }
          default:
            throw new f.JOSENotSupported('Invalid or unsupported "alg" (JWE Algorithm) header value');
        }
      }
      var r = c(8629), s = c(2072), t = c(4070), u = c(6144), v = c(3910);
      async function w(a2, b2, c2) {
        let i2, j2, k2, l2, n2, o2, p2;
        if (!(0, h.G)(a2)) throw new f.JWEInvalid("Flattened JWE must be an object");
        if (void 0 === a2.protected && void 0 === a2.header && void 0 === a2.unprotected) throw new f.JWEInvalid("JOSE Header missing");
        if (void 0 !== a2.iv && "string" != typeof a2.iv) throw new f.JWEInvalid("JWE Initialization Vector incorrect type");
        if ("string" != typeof a2.ciphertext) throw new f.JWEInvalid("JWE Ciphertext missing or incorrect type");
        if (void 0 !== a2.tag && "string" != typeof a2.tag) throw new f.JWEInvalid("JWE Authentication Tag incorrect type");
        if (void 0 !== a2.protected && "string" != typeof a2.protected) throw new f.JWEInvalid("JWE Protected Header incorrect type");
        if (void 0 !== a2.encrypted_key && "string" != typeof a2.encrypted_key) throw new f.JWEInvalid("JWE Encrypted Key incorrect type");
        if (void 0 !== a2.aad && "string" != typeof a2.aad) throw new f.JWEInvalid("JWE AAD incorrect type");
        if (void 0 !== a2.header && !(0, h.G)(a2.header)) throw new f.JWEInvalid("JWE Shared Unprotected Header incorrect type");
        if (void 0 !== a2.unprotected && !(0, h.G)(a2.unprotected)) throw new f.JWEInvalid("JWE Per-Recipient Unprotected Header incorrect type");
        if (a2.protected) try {
          let b3 = (0, d.decode)(a2.protected);
          i2 = JSON.parse(r.D0.decode(b3));
        } catch {
          throw new f.JWEInvalid("JWE Protected Header is invalid");
        }
        if (!(0, g.f)(i2, a2.header, a2.unprotected)) throw new f.JWEInvalid("JWE Protected, JWE Unprotected Header, and JWE Per-Recipient Unprotected Header Parameter names must be disjoint");
        let w2 = { ...i2, ...a2.header, ...a2.unprotected };
        if ((0, s.n)(f.JWEInvalid, /* @__PURE__ */ new Map(), c2?.crit, i2, w2), void 0 !== w2.zip) throw new f.JOSENotSupported('JWE "zip" (Compression Algorithm) Header Parameter is not supported.');
        let { alg: x, enc: y } = w2;
        if ("string" != typeof x || !x) throw new f.JWEInvalid("missing JWE Algorithm (alg) in JWE Header");
        if ("string" != typeof y || !y) throw new f.JWEInvalid("missing JWE Encryption Algorithm (enc) in JWE Header");
        let z = c2 && (0, t.F)("keyManagementAlgorithms", c2.keyManagementAlgorithms), A = c2 && (0, t.F)("contentEncryptionAlgorithms", c2.contentEncryptionAlgorithms);
        if (z && !z.has(x) || !z && x.startsWith("PBES2")) throw new f.JOSEAlgNotAllowed('"alg" (Algorithm) Header Parameter value not allowed');
        if (A && !A.has(y)) throw new f.JOSEAlgNotAllowed('"enc" (Encryption Algorithm) Header Parameter value not allowed');
        if (void 0 !== a2.encrypted_key) try {
          j2 = (0, d.decode)(a2.encrypted_key);
        } catch {
          throw new f.JWEInvalid("Failed to base64url decode the encrypted_key");
        }
        let B = false;
        "function" == typeof b2 && (b2 = await b2(i2, a2), B = true), (0, v.y)("dir" === x ? y : x, b2, "decrypt");
        let C = await (0, u.l)(b2, x);
        try {
          k2 = await q(x, C, j2, w2, c2);
        } catch (a3) {
          if (a3 instanceof TypeError || a3 instanceof f.JWEInvalid || a3 instanceof f.JOSENotSupported) throw a3;
          k2 = (0, m.D)(y);
        }
        if (void 0 !== a2.iv) try {
          l2 = (0, d.decode)(a2.iv);
        ...
        if (void 0 !== a2.tag) try {
          n2 = (0, d.decode)(a2.tag);
        } catch {
          throw new f.JWEInvalid("Failed to base64url decode the tag");
        }
        let D = void 0 !== a2.protected ? (0, r.lF)(a2.protected) : new Uint8Array();
        o2 = void 0 !== a2.aad ? (0, r.xW)(D, (0, r.lF)("."), (0, r.lF)(a2.aad)) : D;
        try {
          p2 = (0, d.decode)(a2.ciphertext);
        } catch {
          throw new f.JWEInvalid("Failed to base64url decode the ciphertext");
        }
        let E = { plaintext: await (0, e.Y)(y, k2, p2, l2, n2, o2) };
        if (void 0 !== a2.protected && (E.protectedHeader = i2), void 0 !== a2.aad) try {
          E.additionalAuthenticatedData = (0, d.decode)(a2.aad);
        } catch {
          throw new f.JWEInvalid("Failed to base64url decode the aad");
        }
        return (void 0 !== a2.unprotected && (E.sharedUnprotectedHeader = a2.unprotected), void 0 !== a2.header && (E.unprotectedHeader = a2.header), B) ? { ...E, key: C } : E;
      }
    }, 9663: (a, b, c) => {
      "use strict";
      c.r(b), c.d(b, { JOSEAlgNotAllowed: () => g, JOSEError: () => d, JOSENotSupported: () => h, JWEDecryptionFailed: () => i, JWEInvalid: () => j, JWKInvalid: () => m, JWKSInvalid: () => n, JWKSMultipleMatchingKeys: () => p, JWKSNoMatchingKey: () => o, JWKSTimeout: () => q, JWSInvalid: () => k, JWSSignatureVerificationFailed: () => r, JWTClaimValidationFailed: () => e, JWTExpired: () => f, JWTInvalid: () => l });
      class d extends Error {
        static code = "ERR_JOSE_GENERIC";
        code = "ERR_JOSE_GENERIC";
        constructor(a2, b2) {
          super(a2, b2), this.name = this.constructor.name, Error.captureStackTrace?.(this, this.constructor);
        }
      }
      class e extends d {
        ...
        reason;
        payload;
        constructor(a2, b2, c2 = "unspecified", d2 = "unspecified") {
          super(a2, { cause: { claim: c2, reason: d2, payload: b2 } }), this.claim = c2, this.reason = d2, this.payload = b2;
        }
      }
      class f extends d {
      ...
      class i extends d {
        ...
        constructor(a2 = "decryption operation failed", b2) {
          super(a2, b2);
        }
      }
      class j extends d {
      ...
      class o extends d {
        ...
        constructor(a2 = "no applicable key found in the JSON Web Key Set", b2) {
          super(a2, b2);
        }
      }
      class p extends d {
        ...
        constructor(a2 = "multiple matching keys found in the JSON Web Key Set", b2) {
          super(a2, b2);
        }
      }
      class q extends d {
        ...
        constructor(a2 = "request timed out", b2) {
          super(a2, b2);
        }
      }
      class r extends d {
        ...
        constructor(a2 = "signature verification failed", b2) {
          super(a2, b2);
        }
      }
    }, 9814: (a, b, c) => {
      "use strict";
      c.d(b, { $: () => h, Y: () => g });
      let d = (a2, b2 = "algorithm.name") => TypeError(`CryptoKey does not support this operation, its ${b2} must be ${a2}`);
      function e(a2) {
        return parseInt(a2.name.slice(4), 10);
      }
      function f(a2, b2) {
        if (b2 && !a2.usages.includes(b2)) throw TypeError(`CryptoKey does not support this operation, its usages must include ${b2}.`);
      }
      function g(a2, b2, c2) {
        switch (b2) {
          case "HS256":
          case "HS384":
          case "HS512": {
            if ("HMAC" !== a2.algorithm.name) throw d("HMAC");
            let c3 = parseInt(b2.slice(2), 10);
            if (e(a2.algorithm.hash) !== c3) throw d(`SHA-${c3}`, "algorithm.hash");
            break;
          }
          case "RS256":
          case "RS384":
          case "RS512": {
            if ("RSASSA-PKCS1-v1_5" !== a2.algorithm.name) throw d("RSASSA-PKCS1-v1_5");
            let c3 = parseInt(b2.slice(2), 10);
            if (e(a2.algorithm.hash) !== c3) throw d(`SHA-${c3}`, "algorithm.hash");
            break;
          }
          ...
          case "PS384":
          case "PS512": {
            if ("RSA-PSS" !== a2.algorithm.name) throw d("RSA-PSS");
            let c3 = parseInt(b2.slice(2), 10);
            if (e(a2.algorithm.hash) !== c3) throw d(`SHA-${c3}`, "algorithm.hash");
            break;
          }
          case "Ed25519":
          case "EdDSA":
            if ("Ed25519" !== a2.algorithm.name) throw d("Ed25519");
            break;
          case "ML-DSA-44":
          case "ML-DSA-65":
          case "ML-DSA-87":
            let g2;
            if (g2 = a2.algorithm, g2.name !== b2) throw d(b2);
            break;
          case "ES256":
          case "ES384":
          case "ES512": {
            if ("ECDSA" !== a2.algorithm.name) throw d("ECDSA");
            let c3 = function(a3) {
              switch (a3) {
                case "ES256":
                  return "P-256";
            ...
            if (a2.algorithm.namedCurve !== c3) throw d(c3, "algorithm.namedCurve");
            break;
          }
          default:
      ...
      function h(a2, b2, c2) {
        switch (b2) {
          ...
          case "A192GCM":
          case "A256GCM": {
            if ("AES-GCM" !== a2.algorithm.name) throw d("AES-GCM");
            let c3 = parseInt(b2.slice(1, 4), 10);
            if (a2.algorithm.length !== c3) throw d(c3, "algorithm.length");
            break;
          }
          case "A128KW":
          case "A192KW":
          case "A256KW": {
            if ("AES-KW" !== a2.algorithm.name) throw d("AES-KW");
            let c3 = parseInt(b2.slice(1, 4), 10);
            if (a2.algorithm.length !== c3) throw d(c3, "algorithm.length");
            break;
          }
          case "ECDH":
            switch (a2.algorithm.name) {
              case "ECDH":
              case "X25519":
                break;
              default:
                throw d("ECDH or X25519");
            }
            break;
          case "PBES2-HS256+A128KW":
          case "PBES2-HS384+A192KW":
          case "PBES2-HS512+A256KW":
            if ("PBKDF2" !== a2.algorithm.name) throw d("PBKDF2");
            break;
          case "RSA-OAEP":
          case "RSA-OAEP-256":
          case "RSA-OAEP-384":
          case "RSA-OAEP-512": {
            if ("RSA-OAEP" !== a2.algorithm.name) throw d("RSA-OAEP");
      ...
      var d = c(9814);
      function e(a2, b2) {
        if (a2.algorithm.length !== parseInt(b2.slice(1, 4), 10)) throw TypeError(`Invalid key size for alg: ${b2}`);
      }
      function f(a2, b2, c2) {
        return a2 instanceof Uint8Array ? crypto.subtle.importKey("raw", a2, "AES-KW", true, [c2]) : ((0, d.$)(a2, b2, c2), a2);
      }
      async function g(a2, b2, c2) {
        let d2 = await f(b2, a2, "wrapKey");
        e(d2, a2);
        let g2 = await crypto.subtle.importKey("raw", c2, { hash: "SHA-256", name: "HMAC" }, true, ["sign"]);
        return new Uint8Array(await crypto.subtle.wrapKey("raw", g2, d2, "AES-KW"));
      }
      async function h(a2, b2, c2) {
        let d2 = await f(b2, a2, "unwrapKey");
        e(d2, a2);
        let g2 = await crypto.subtle.unwrapKey("raw", c2, d2, "AES-KW", { hash: "SHA-256", name: "HMAC" }, true, ["sign"]);
        return new Uint8Array(await crypto.subtle.exportKey("raw", g2));
      }
    } }, (a) => {
      var b = a(a.s = 259);
...
var init_edgeFunctionHandler = __esm({
  "node_modules/@opennextjs/aws/dist/core/edgeFunctionHandler.js"() {
    ...
    globalThis._ROUTES = [{ "name": "src/middleware", "page": "/", "regex": ["^\\/app(?:\\/(_next\\/data\\/[^/]{1,}))?(?:\\/((?!_next\\/static|_next\\/image|favicon.ico).*))(\\.json)?[\\/#\\?]?$"] }];
